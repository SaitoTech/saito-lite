...
***REMOVED*** Create the event's object if not yet created
        if (!this.hop.call(this.events, event)) {
          this.events[event] = [];
    ***REMOVED***

***REMOVED*** Add the handler to queue
        var index = this.events[event].push(handler) - 1;

***REMOVED*** Provide handle back for removal of event
        return {
...
***REMOVED*** If the event doesn't exist, or there's no handlers in queue, just leave
        if (!this.hop.call(this.events, event)) {
          return;
    ***REMOVED***

***REMOVED*** Cycle through events queue, fire!
...
***REMOVED*** If the `bound` option is acitve, a maximum running distance should be
***REMOVED*** reduced by `perView` and `focusAt` settings. Running distance
***REMOVED*** should end before creating an empty space after instance.
