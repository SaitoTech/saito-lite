...
class Wallet {
  ...
  constructor(app) {
    ...
    this.wallet.preferred_crypto	 = "SaitoCrypto";
...
console.log("---------------------");
  ...
  returnPreferredCrypto() {
    return this.returnCryptoModuleByName(this.wallet.preferred_crypto)
  }
  returnCryptoModuleByName(modname) {
    let mods = this.returnAvailableCryptos();
    for (let i = 0; i < mods.length; i++) {
      if (mods[i].name === modname) { return mods[i]; }
    }
    throw "Module Not Found: " + modname;
  }
  setPreferredCrypto(modname="Saito") {
    this.wallet.preferred_crypto = modname;
    this.saveWallet();
    this.app.connection.emit("set_preferred_crypto", modname);
  }
  ///////////////////////////
