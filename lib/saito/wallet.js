...
class Wallet {
  ...
  constructor(app) {
    ...
    class SaitoCrypto extends AbstractCryptoModule {
      ...
      //
      // TODO - triggers too easily because slips do not 
      // have timestamp information. We need more complicated
      // logic to handle Saito payments reliably.
      //
	...
	if (to == this.app.wallet.returnPublicKey()) {
	  for (let i = 0; i < this.app.wallet.wallet.inputs.length; i++) {
	    if (this.app.wallet.wallet.inputs[i].amt === howMuch) {
	      if (this.app.wallet.wallet.inputs[i].ts >= timestamp) {
		for (let ii = 0; ii < this.app.wallet.wallet.inputs[i].from_slips.length; ii++) {
		  if (this.app.wallet.wallet.inputs[i].from_slips[ii].add == from) {
console.log("payment is in input list...");
 	            return 1; 
	          }
	        }
	      }
	  ...
	  for (let i = 0; i < this.app.wallet.wallet.outputs.length; i++) {
	    if (this.app.wallet.wallet.outputs[i].amt === howMuch) {
	      if (this.app.wallet.wallet.outputs[i].ts >= timestamp) {
		for (let ii = 0; ii < this.app.wallet.wallet.outputs[i].from_slips.length; ii++) {
		  if (this.app.wallet.wallet.outputs[i].from_slips[ii].add == from) {
console.log("payment is in output list...");
 	            return 1; 
	          }
	        }
	      }
  ...
  processPayments(blk, lc=0) {
    ...
    for (let i = 0; i < blk.transactions.length; i++) {
      ...
      for (let ii = 0; ii < to_slips.length; ii++) {
        ...
        to_slips[ii].lc  = lc;
console.log("TS set to : " +blk.block.ts);
...
console.log("---------------------");
  ...
  setPreferredCrypto(ticker) {
    ...
    // console.log("set_preferred_crypto wallet.js", mods, ticker, can_we_do_this);
  ...
  */
  async sendPayment(senders=[], receivers=[], amounts=[], timestamp, mycallback, ticker) {
    ...
    if (!this.doesPreferredCryptoTransactionExist(senders, receivers, amounts, timestamp, ticker)) {
      ...
      console.log("from " + senders[0]);
      console.log("to " + receivers[0]);
      let cryptomod = this.returnCryptoModuleByTicker(ticker);
      for(let i = 0; i < senders.length; i++) {
        if(senders[i] === cryptomod.returnAddress()) {
          ...
          try {
            ...
            let hash = await cryptomod.transfer(amounts[i]/1000, receivers[i]);
   ...
   * @param {Function} mycallback - (Array of {address: {String}, balance: {Int}}) -> {...}
   * @param {String} ticker - Ticker of install crypto module
   * @param {Int} tries - (default: 36) Number of tries to query the underlying crypto API before giving up
   * @param {Int} pollWaitTime - (default: 5000) Amount of time to wait between tries
   * @return {Array} Array of {address: {String}, balance: {Int}}
   */
  receivePayment(senders=[], receivers=[], amounts=[], timestamp, mycallback, ticker, tries = 36, pollWaitTime = 5000) {

console.log("RECEIVE PAYMENT A");

    return new Promise(async(resolve, reject) => {

console.log("RECEIVE PAYMENT B");
...
console.log("PAST B+");
      ...
      let cryptomod = this.returnCryptoModuleByTicker(ticker);
console.log("which cryptomod: " + cryptomod.name);
      await cryptomod.onIsInitialized();
console.log("which cryptomod?");
      console.log("**** receivePayment ****");
      console.log("from " + senders[0]);
      console.log("to " + receivers[0]);
      console.log("me " + cryptomod.returnAddress());
...
console.log("checkingpayment...");
        return await cryptomod.hasPayment(amounts[0]/1000, senders[0], receivers[0], timestamp[0]); 
      }

console.log("RECEIVE PAYMENT D");
...
console.log("RECEIVE PAYMENT E");
      ...
      let did_complete_payment = (result) => {
        if (result) {
          ...
          tries--;
          if(tries > 0) {
            // if there are more tries left, re-poll after pollWaitTime
            setTimeout(() => {
