...
//console.log("loop: " + i + " - " + this.wallet.pending.length);
  ...
  signTransaction(tx) {
    ...
    // HACK
    //
    let x = Buffer.from(JSON.stringify(tx.transaction.msg)).toString('ascii');
    let y = Buffer.from(JSON.stringify(tx.transaction.msg)).toString('utf8');
    if (x != y) {
      //
      // force a validation failure
      // 
      tx.transaction.sig = "invalid tx: contains more than ascii"; 
      return 0;
    }
