...
console.log("trying to rebroadcast the pending txs...");  
    ...
    for (let i = 0; i < this.wallet.pending.length; i++) {
      ...
      if (!tx.isFrom(this.returnPublicKey())) {
        ...
        if (tx.transaction.type == 0) {
          if (peer == null) {
console.log("trying to rebroadcast the pending txs... 2");  
            this.app.network.propagateTransaction(tx);
          } else {
console.log("trying to rebroadcast the pending txs... 3");  
            this.app.network.propagateTransaction(tx);
            }
        } else {
          //
