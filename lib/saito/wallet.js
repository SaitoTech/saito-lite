  ...
  signAndEncryptTransaction(tx) {
    ...
    try {
      ...
      if (this.app.keys.hasSharedSecret(tx.transaction.to[0].add)) {
        tx.transaction.m = this.app.keys.encryptMessage(tx.transaction.to[0].add, tx.transaction.m);
      }
...
console.log("---------------------");
  ...
  returnAvailableCryptos() {
    ...
    modules.push(this.returnPreferredCrypto("SAITO"));
    return modules;
  }
  ...
  returnPreferredCrypto(ticker="") {
	...
	if (modules[i].ticker === this.wallet.preferred_crypto) {
	  return modules[i];
	}
      }
    }
  ...
  returnPreferredCryptoByName(modname="Saito") { 
    ...
    for (let i = 0; i < mods.length; i++) {
      if (mods[i].name === modname) { return mods[i]; }
    }
    return this.returnPreferredCrypto();
  }
  ...
  // access functions
  //
  // we can avoid these helper functions, but we need to be systematic about variable names 
  // and conventions "returnX" rather than "getX" and have the protocol set for how the 
  // functions actually work. right now the saito-header component uses these in order to 
  // avoid having it access module contents directly.
  //
  async returnPreferredCryptoAddress() { return await this.returnPreferredCrypto().getAddress(); }
  async returnPreferredCryptoBalance() { return await this.returnPreferredCrypto().getBalance(); }
  returnPreferredCryptoTicker()        { return this.returnPreferredCrypto().ticker; }
  returnPreferredCryptoName()        { return this.returnPreferredCrypto().name; }

  setPreferredCryptoByModuleName(modname="Saito") {
    if (modname == "Saito") {
      this.setPreferredCrypto("SAITO");
      return;
    }
    let cryptomod = this.app.modules.returnModule(modname);
    if (cryptomod) {
      if (cryptomod.ticker) {
        this.setPreferredCrypto(cryptomod.ticker);
	return;
      }
    }
  }
  setPreferredCrypto(ticker="SAITO") {
