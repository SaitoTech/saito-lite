...
console.log("---------------------");
  ...
  async returnPreferredCryptoBalances(addresses=[], ticker="") {
    return new Promise((resolve, reject) => {
      if (ticker == "") { ticker = this.wallet.preferred_crypto; }
      let cryptomod = this.returnCryptoModuleByTicker(ticker);
      let balanceCallbacks = [];
      for (let i = 0; i < addresses.length; i++) {
        // returnBalance is async, this is a list of promises not results.
        balanceCallbacks.push(cryptomod.returnBalance(addresses[i]));
      }
      Promise.all(balanceCallbacks).then((balances) => {
        let returnObj = [];
        for (let i = 0; i < addresses.length; i++) {
