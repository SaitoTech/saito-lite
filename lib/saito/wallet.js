...
console.log("UPDATE TRIGGERED: " + this.app.options.wallet.version + " -- " + this.wallet.version);
...
//console.log("loop: " + i + " - " + this.wallet.pending.length);
...
console.log("####################");
  ...
  updateBalance() {
    ...
    this.saitoCryptoInterface.eventEmitter.emit("balance_change", this.wallet.balance);
  }
  ...
  createUnsignedTransaction(publickey="", amt=0.0, fee=0.0) {
    ...
    //
    if (this.wallet.inputs.length > 200) {

console.log("---------------------");
console.log("Merging Wallet Slips!");
console.log("---------------------");

      let slips_to_merge = 5;
      let slips_merged = 0;
  ...
  getSupportedCryptos() {
    let modules = this.app.modules.getRespondTos("is_cryptocurrency");
    modules.push(this.saitoCryptoInterface);
    return modules;
  }
  getCryptoModuleInterfaceByName(modname) {
    if(modname === "SaitoCrypto") {
      return this.saitoCryptoInterface;
    } else {
      return this.app.modules.returnModule(modname).respondTo("is_cryptocurrency");
    }
  }
  getPreferredCryptoName() {
    return this.returnPreferredCryptoModule().name;
  }
  getPreferredCryptoDescription() {
    return this.returnPreferredCryptoModule().description;
  }
  getPreferredCryptoTicker() {
    return this.returnPreferredCryptoModule().ticker;
  }
  async getPreferredCryptoBalance() {
    return await this.returnPreferredCryptoModule().getBalance();
  }
  async doPreferredCryptoTransfer(howMuch, to) {
    return await this.returnPreferredCryptoModule().transfer(howMuch, to);
  }
  async getPreferredCryptoAddress() {
    return await this.returnPreferredCryptoModule().getAddress();
  }
  getPreferredCryptoEventEmitter() {
    return this.returnPreferredCryptoModule().eventEmitter;
  }
  getPreferredCryptoFeeEstimate() {
    return this.returnPreferredCryptoModule().estimateFee();
  }
  setConfirmationThreshold(numberOfConfirmations) {
    // TODO: Implement me!
  }
  subscribeToPreferredCryptoBalanceChangeEvent(callback) {
    this.returnPreferredCryptoModule().eventEmitter.on("balance_change", callback);
  }
  unsubscribeFromPreferredCryptoBalanceChangeEvent(callback) {
    this.returnPreferredCryptoModule().eventEmitter.removeListener("balance_change", callback);
  }
  subscribeToPreferredCryptoConfirmedBalanceChangeEvent(callback) {
    this.returnPreferredCryptoModule().eventEmitter.on("confirmed_balance_change", callback);
  }
  unsubscribeFromPreferredCryptoConfirmedBalanceChangeEvent(callback) {
    this.returnPreferredCryptoModule().eventEmitter.removeListener("confirmed_balance_change", callback);
  }
  subscribeToSaitoBalanceChangeEvent(callback) {
    this.saitoCryptoInterface.eventEmitter.on("balance_change", callback);
  }
  unsubscribeFromSaitoBalanceChangeEvent(callback) {
    this.saitoCryptoInterface.eventEmitter.removeListener("balance_change", callback);
  }
  
  initSaitoCryptoInterface() {
    this.saitoCryptoInterface = {
      name: "SaitoCrypto",
      description: "Saito",
      ticker: "SAITO",
      getBalance: async() => {
      ...
      estimateFee: null, // TODO: Implement me!
      setConfirmationThreshold: null, // TODO: Implement me!
    }
  }
  returnPreferredCryptoModule() {
    let preferredCrypto = this.app.options.wallet.preferred_crypto ? this.app.options.wallet.preferred_crypto : "SaitoCrypto";
    return this.getCryptoModuleInterfaceByName(preferredCrypto);  
  }
  setPreferredCrypto(cryptoModuleName) {
    // this.returnPreferredCryptoModule().eventEmitter.removeAllListeners("balance_change");
    // this.returnPreferredCryptoModule().eventEmitter.removeAllListeners("confirmed_balance_change");
    this.app.options.wallet.preferred_crypto = cryptoModuleName;
    this.app.storage.saveOptions();
    this.app.storage.eventEmitter.emit("preferred_crypto_set");
  }
  subscribeToPreferredCryptoChangeEvent(callback) {
    this.app.storage.eventEmitter.on("preferred_crypto_set", callback);
  }
  unsubscribeFromPreferredCryptoChangeEvent(callback) {
    this.app.storage.eventEmitter.removeListener("preferred_crypto_set", callback);
  }
  
  ///////////////////////////
  // END NON-SAITO CRYPTOS //
  ///////////////////////////
}
