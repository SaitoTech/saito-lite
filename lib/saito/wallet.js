...
console.log(err);
  ...
  createUnsignedTransaction(publickey="", amt=0.0, fee=0.0, force_merge=0) {
    ...
    if (total_fees.gt(wallet_avail)) {
      amt = 0.0;
      fee = 0.0;
      console.log("Inadequate funds in wallet to create transaction");
      return null;
    }
...
console.log("---------------------");
  ...
  createReplacementTransaction(oldtx) {
    ...
    let newtx = new transaction(oldtx.transaction);
    let inputs = this.returnAdequateInputs(inputs_amt);
    if(inputs) {
      newtx.transaction.inputs = this.returnAdequateInputs(inputs_amt);
      this.saveWallet();
      return newtx;  
    } else {
      return null;
