...
console.log("---------------------");
  ...
  // NON-SAITO CRYPTOS //
  ///////////////////////
  returnSupportedCryptos() {
    let cryptos = ["SAITO"];
    for (let i = 0; i < this.app.modules.mods.length; i++) {
      let is_cryptocurrency = this.app.modules.mods[i].respondTo("is_cryptocurrency");
      if (is_cryptocurrency != null) {
	cryptos.push(is_cryptocurrency.ticker);
      }
    }
  }

  returnPreferredCrypto() {
    if (this.app.options.wallet.default_crypto) {
      return this.app.options.wallet.default_crypto;
    }
    return "SAITO";
  }

  async returnCryptoBalance(crypto) {
    if (crypto == "SAITO") {
    ...
    for (let i = 0; i < this.app.modules.mods.length; i++) {
      let is_cryptocurrency = this.app.modules.mods[i].respondTo("is_cryptocurrency");
      if (is_cryptocurrency != null) {
	if (is_cryptocurrency.ticker === crypto) {
	  return await is_cryptocurrency.returnCryptoBalance();
	}
      }
    }
    return 0;
