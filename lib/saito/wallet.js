...
class Wallet {
  ...
  constructor(app) {
    ...
    class SaitoCrypto extends AbstractCryptoModule {
      ...
      async returnAddress() {
alert("returning address");
        return this.app.wallet.returnPublicKey();
      }
      async transfer(howMuch, to) {
alert("making tx");
	let newtx = this.app.wallet.createUnsignedTransactionWithDefaultFee(to, howMuch);
alert("tx made: ");
	if (newtx == null) { 
	  throw "Unable to create valid transaction with default fee";
	  return;
        }
alert("tx propagating: ");
