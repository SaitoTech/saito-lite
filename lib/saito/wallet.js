...
class Wallet {
  ...
  doesSlipInPendingTransactionsSpendBlockHash(bsh="") {
    console.log("doesSlipInPendingTransactionsSpendBlockHash");
    console.log("bsh: " + bsh);
    for (let i = 0; i < this.wallet.pending.length; i++) {
      ...
      for (let k = 0; k < ptx.transaction.from.length; k++) {
        console.log("ptx bsh: " + k + " : " + ptx.transaction.from[k].bsh);
        if (ptx.transaction.from[k].bsh == bsh) {
          return true;
        }
      }
    }
    // TODO put this back to return false. just want to test this for cleaning up existing pending 
    // return false;
    return true;
  }
...
console.log(err);
  ...
  createUnsignedTransactionWithDefaultFee(publickey="", amt=0.0, force_merge=0) {
    ...
    if (Big(this.wallet.default_fee).gt(wallet_avail)) {
      console.log("Inadequate funds in wallet for default fee, creating with 0.0 fee instead.");
      return this.createUnsignedTransaction(publickey, amt, 0.0);
    } else {
...
console.log("set preferred crypto: " + ticker);
