...
try {
alert("ADDING TO PENDING: " + JSON.stringify(tx.transaction.msg));
} catch (err) {
}
    this.wallet.pending.push(JSON.stringify(tx.transaction));
    this.saveWallet();
  }
...
console.log("INPUTS: " + JSON.stringify(this.wallet.inputs));
console.log("SPENDS: " + JSON.stringify(this.wallet.spends));
    return null;
  }
  ...
  resetWallet() {
    ...
    if (this.app.options.blockchain != undefined) {
      this.app.options.blockchain.last_bid = "";
      this.app.options.blockchain.last_bsh = "";
      this.app.options.blockchain.last_ts = 0;
      this.app.options.blockchain.last_bf = 0.0;
      this.app.options.blockchain.lowest_acceptable_ts = 0;
      this.app.options.blockchain.lowest_acceptable_bsh = 0;
      this.app.options.blockchain.lowest_acceptable_bid = 0;
    }   
