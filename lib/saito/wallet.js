...
class Wallet {
  ...
  constructor(app) {
    ...
    this.wallet.version                  = 3.295;

    this.wallet.preferred_crypto	 = "SAITO";
    this.wallet.preferred_txs		 = [];

    this.preferred_return_balance_timer  = null;
    this.preferred_return_balance_speed  = 5000;
    this.preferred_return_balance_active = false;

    this.preferred_send_payment_timer    = null;
    this.preferred_send_payment_speed    = 5000;
    this.preferred_send_payment_active   = false;

    this.preferred_receive_payment_timer = null;
    this.preferred_receive_payment_speed = 5000;
    this.preferred_receive_payment_active = false;
...
console.log("---------------------");
  ...
  returnPreferredCrypto() {
    ...
    if (this.wallet.preferred_crypto === "DOT" || this.wallet.preferred_crypto === "KSM" || this.wallet.preferred_crypto === "WESTIE" || this.wallet.preferred_crypto === "SAITO"){
      this.setPreferredCrypto(this.saitoCrypo.name);
    }
    try {
  ...
  // optional.
  //
  async returnBalance(addresses=[], mycallback=null, ticker="") {
    ...
    for (let i = 0; i < robj.addresses.length; i++) {
      robj.balance.push(0);
      robj.status.push(0);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.preferred_return_balance_active = true;
      this.preferred_return_balance_timer = setInterval() => {

	if (active_querying_api == 0) {
	  active_querying_api = 1;
	  ...
	  if (can_we_resolve == 1) {
	    clearInterval(this.preferred_return_balance_timer);
	    active_querying_api = 0;
      ...
      }, this.preferred_return_balance_speed);
