  ...
  **************************************/
  async returnDatabaseByName(dbname) {
    ...
    try {
      ...
      logger.info("Error creating database for db-name: " + dbname);
      return null;
      ...
      try {
        ...
        if (fileID !== "empty") {
          ...
          if (blk == null || blk.is_valid == 0) {
            logger.info("We have saved an invalid block: " + fileID);
            return null;
          }
  ...
  async saveBlock(blk=null, lc=0) {
    try {
      ...
      logger.info("ERROR 285029: error saving block to disk " + err);
    }
    return true;
  }
  ...
  loadBlockByFilename(filename) {
    ...
    if (fs.existsSync(block_filename)) {
      ...
      logger.error(`cannot open: ${block_filename} as it does not exist on disk`)
      return null;
    }
    } catch (err) {
      logger.info("Error reading block from disk");
      logger.error(err);
    }
  ...
  async returnBlockFilenameByHash(block_hash, mycallback) {
    ...
    try {
      ...
      logger.info("ERROR getting block filename in storage: " + err);
      mycallback(null, err);
    }
