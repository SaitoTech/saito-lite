  ...
  async saveBlock(blk=null, lc=0) {
    ...
    try {
      ...
      if (!fs.existsSync(filename)) {
        fs.writeFileSync(filename, blk.returnBlockFileData(), this.file_encoding_save);
      }
      return true;
    } catch (err) {
...
if (data.block.id == 67) {
  for (let i = 0; i < data.transactions.length; i++) {
    if (data.transactions[i].transaction.id == 351) {

let checkobj = data.transactions[i].transaction.msg;

const buf = Buffer.from(JSON.stringify(checkobj));
console.log("\n\n\nVALID UTF8: " + isValidUTF8(buf));
