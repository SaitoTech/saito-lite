...
class Server {
  ...
  // const io          = require('socket.io')(webserver);
    ...
    app.post('/blocks', async (req, res) => {
      ...
      try {
        var body = JSON.parse(this.app.crypto.base64ToString(Object.keys(req.body)[0]));
        var block_hashes = body.blocks;
...
console.log(bsh + " -- " + pkey);

      let blk = await this.app.blockchain.returnBlockByHash(bsh);
      let newblk = blk.returnLiteBlock(bsh, keylist);

console.log("LITE BLOCK IS: ");
console.log(JSON.stringify(newblk.block));
console.log(JSON.stringify(newblk.transactions));
...
console.log("request for which publickey?");
...
console.log("keylist for processing: " + JSON.stringify(keylist));

      let latest_bid = this.app.blockchain.last_bid;
      let sync_from_bid = latest_bid-10;

      try {

        blocks = block_hashes.map(async bhash => {

          let blk = await this.app.blockchain.returnLiteBlock(bsh, keylist);
