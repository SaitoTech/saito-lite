...
const io          = require('socket.io')(webserver, {cookie: false});
const fs          = require('fs');
...
*/
      this.server.sendblks = (typeof this.server.sendblks == "undefined") ? 1 : this.app.options.server.sendblks;
      this.server.sendtxs = (typeof this.server.sendtxs == "undefined") ? 1 : this.app.options.server.sendtxs;
      this.server.sendgts = (typeof this.server.sendgts == "undefined") ? 1 : this.app.options.server.sendgts;
      this.server.receiveblks = (typeof this.server.receiveblks == "undefined") ? 1 : this.app.options.server.receiveblks;
      this.server.receivetxs = (typeof this.server.receivetxs == "undefined") ? 1 : this.app.options.server.receivetxs;
      this.server.receivegts = (typeof this.server.receivegts == "undefined") ? 1 : this.app.options.server.receivegts;
    }
	...
	res.write(new Buffer(newblk.returnBlockHeaderData(), 'utf8'), 'utf8');
        for (let i = 0; i < blk.transactions.length; i++) {
          res.write(new Buffer("\n", 'utf8'), 'utf8');
          res.write(new Buffer(JSON.stringify(blk.transactions[i]), 'utf8'), 'utf8');
        }
	res.end();
