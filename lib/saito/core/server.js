...
console.log("OUR KEYLIST FOR THIS PEER: " + JSON.stringify(keylist));

      let blk = await this.app.blockchain.returnBlockByHashFromMemoryOrDisk(bsh);
      if (blk == null) {
        res.send("{}");
        return;
      } else {
        let newblk = blk.returnLiteBlock(bsh, keylist);
        res.send(newblk.returnBlockFileData());
        return;
      }
