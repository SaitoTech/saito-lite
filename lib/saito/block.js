...
console.log("GOLDEN TICKET: " + this.containsGoldenTicket());
  ...
  async validate() {
    ...
    //
    // calculate when staking happens
    //
    //let st = Big(prevblk.block.st).plus(Big(prevblk.block.reclaimed));
    //let sc = Big(bt).div(this.app.blockchain.genesis_period).toFixed(8);
    //    st = bt.minus(Big(cb)).toFixed(8);
