...
class Block {
  ...
  async bundle(prevblk=null) {
    ...
    if (prevblk != null) {
      ...
      this.block.ss             = prevblk.block.ss;
      this.block.sa             = prevblk.block.sa;
...
console.log("GOLDEN TICKET: " + this.containsGoldenTicket());
    ...
    if (prevblk != null) {
      ...
      if (payments.staker_pos != -1) { 
        this.app.blockchain.stakers_current++;
	let staker_slip = this.app.blockchain.index.stakers[payments.staker_pos][payments.staker_sid];
	staker_slip.pending = 1; 
	normalized_staker_payout = this.returnNormalizedStakerPayment(staker_slip.slip.amt);

console.log("\n\n\n");
console.log(staker_slip.amt + " -- " + this.block.sa + " -- " + this.block.ss);
console.log("CALCULATED NORMAL STAKER PAYOUT AS: " + normalized_staker_payout);
