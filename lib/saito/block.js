...
console.log("CALCULATING NSP:");
console.log(staker_slip.amt);
console.log(this.sc);
console.log(prevblk.block.bif);
console.log(prevblk.block.sa);
console.log(prevblk.block.ss);
...
console.log("DONE CALCULATING NSP");
...
console.log("\n\nVALIDATING BLOCK " + this.block.id);
  ...
  returnMerkleRoot() {
    ...
    for (let i = 0; i < this.transactions.length; i++) { 
      if (this.transactions[i].transaction.type == 9) {
        ...
        console.log("MR: PUSHING SIG: " + this.app.crypto.hash(this.transactions[i].returnSignatureSource(this.app)));
console.log("based on signature source: " + this.transactions[i].returnSignatureSource(this.app));
console.log("full tx dump: " + JSON.stringify(this.transactions[i].transaction));
   ...
   while (mr == "") {
      ...
      if (txs.length <= 2) {
	if (txs.length == 1) {
console.log("mr set to: " + txs[0]);
	  mr = txs[0];
	} else {
	  console.log("combine 1a: " + txs[0]);
	  console.log("combine 1b: " + txs[1]);
	  console.log("combine 1c: " + this.app.crypto.hash((""+txs[0]+txs[1])));
	  mr = this.app.crypto.hash((""+txs[0]+txs[1]));
	}
      } else {

        for (let i = 0; i < txs.length; i++) {
console.log("i: " + i + " (lim: " + txs.length-2 + ")");
console.log("t1 --> " + txs[i]);
console.log("t2 --> " + txs[i+1]);
          if (i <= txs.length-2) {
	    console.log("combine 2a: " + txs[i]);
	    console.log("combine 2b: " + txs[i+1]);
	    console.log("combine 2c: " + this.app.crypto.hash((""+txs[i]+txs[i+1])));
	    tx2.push(this.app.crypto.hash((""+txs[i]+txs[i+1])));
	    i++;
	  } else {
	    console.log("non combine: " + txs[i]);
	    tx2.push(txs[i]);
  	  }
        }
