...
console.log("\n\nBUNDLING: " + this.block.id + "(" + this.transactions.length + ")");
...
console.log("STAKER SLIP: " + JSON.stringify(staker_slip));
...
console.log("sa: " + this.block.sa);
    ...
    for (let i = 0; i <= this.transactions.length-1; i++) {
      ...
      if (this.transactions[i].transaction.type == 1) {
	    ...
	    //
	    // mining share calculatd here
	    //
            //let miner_share = Big(this.mc).div(this.block.paysplit).toFixed(8);


            //
            // prevblk is not null here
            //
            let payments = await prevblk.calculateBlockPayments(this.transactions[i].transaction.msg);

console.log("\n\n\nVALIDATE Block Payouts: ");
...
console.log(" ===> " + prevblk.block.st);
console.log(" ===> " + payments.staker_share);
console.log(" ===> " + normalized_staker_payout);
