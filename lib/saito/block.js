...
console.log("GOLDEN TICKET: " + this.containsGoldenTicket());
    ...
    for (let i = 0; i < this.transactions.length; i++) {
	  ...
	  //
	  // handle staking round update if needed
	  //
          if (this.app.blockchain.stakers_current == 0) {
	    this.app.blockchain.updateStakingRound(this.block.id-1);
    	  }

          let payments = prevblk.returnBlockPayouts(this.transactions[i].transaction.msg);

          //
          // reset vent pending on the staking slips so they will validate
          // 
console.log("\n\nBUNDLE BLOCK -- resetting bundling pending for staking slips so that they can be updated in validate/..");
console.log("\n\nSTAKING POOL: " + JSON.stringify(this.app.blockchain.index.staking) + "\n");
console.log("\n\nRETURNED PAYMENTS: " + JSON.stringify(payments));
          if (payments.blocks) {
console.log("T");
            for (let i = 0; i < payments.blocks.length; i++) {
              if (payments.blocks[i].staker_pos != -1) {
console.log("resetting this slip: " + JSON.stringify(this.app.blockchain.index.staking[payments.blocks[i].staker_pos][payments.blocks[i].staker_sid]));
	        this.app.blockchain.index.staking[payments.blocks[i].staker_pos][payments.blocks[i].staker_sid].pending = 1;
	      }
            }
          }
console.log("\n\nDONE WITH RESET");
console.log(JSON.stringify(this.app.blockchain.index.staking));
  ...
  returnBlockPayouts(gt=null, rvalue={}, rloop=0, stakers=[]) {
    ...
    try {
      if (gt == null) {
	    ...
	    //
	    // universal scope variable as we don't want to change the object or it may not validate
	    //
    ...
    if (rloop == 0) {
      ...
      //
      // before we get payments for stakers, we check to see if we
      // need to update the staking pool. we do this here in order
      // to have the block_id
      //
      if (this.stakers_current == 0 && rloop == 1) {

        if (this.stakers_pending == 0) { return rvalue; }
        ...
        this.staking_round_max_bid[this.staking_round] = this.app.blockchain.last_bid;
      ...
      // winning staker
      //
      let x = this.app.blockchain.returnWinningStaker(gt, stakers, this.current_rvalue_for_block_payout_calculation);
      stakers = x.stakers;
...
console.log("WE HAVE DECIDED THAT THE WINNING STAKER IS: " + staker_bid + " - " + staker_sid + " - " + staker_pos + " at rloop: " + rloop);
