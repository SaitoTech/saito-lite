...
class Block {
  ...
  constructor(app, blkjson = "", txsjson = "", confirmations = -1) {
    ...
    this.block.mt 		= Big("2868100000.0");  // mining treasury
    this.block.st 	      	= Big("0.0"); 		// staking treasury
                                                        //
...
console.log("BLOCK ID: " + this.block.id + "(" + this.transactions.length + ")");
console.log("GOLDEN TICKET: " + this.containsGoldenTicket());
    //
	...
	golden_ticket_solution = this.transactions[i].transaction.msg;
...
console.log("SS and SA: " + this.block.ss + " - " + this.block.sa);
    ...
    if (prevblk != null) {
      ...
      //
      let payments = prevblk.calculateBlockPayments(golden_ticket_solution);
      //
      // and mark stake as unpaid
      //
      this.app.blockchain.index.stakers[payments.staker_pos][payments.staker_sid].paid = 0;
      let normalized_staker_payout = this.sc;
	...
	//
	if (this.containsGoldenTicket() == 1) {
	  ...
	  //
	    ...
	    this.block.ss++;
	    this.block.sa = Big(this.block.sa).plus(Big(payments.staker_share)).toFixed(8);

	    let staker_slip = this.app.blockchain.index.stakers[payments.staker_pos][payments.staker_sid];
	    normalized_staker_payout = this.returnNormalizedStakerPayment(staker_slip.slip.amt);
console.log(JSON.stringify(payments));
console.log("_______________________");
console.log("_______  BUNDLE  ______");
console.log("_______________________");
console.log("Amount of Staker Payout: " + staker_slip.slip.amt);
console.log("Normalized Staker Payout: " + normalized_staker_payout);


            //
            // reclaim payment to staking treasury
            //
	    this.block.st = Big(this.block.st).plus(Big(payments.staker_share));
  ...
  returnNormalizedStakerPayment(amt) {
console.log("this.sc: " + this.sc);
console.log("this.block.sa: " + this.block.sa);
console.log("this.block.ss: " + this.block.ss);
    return Big(this.sc).times(Big(amt).div(Big(this.block.sa).div(Big(this.block.ss)))).toFixed(8);
  }
...
console.log("\n\n\n");
console.log("ss: " + this.block.ss);
console.log("sa: " + this.block.sa);
    ...
    for (let i = 0; i <= this.transactions.length-1; i++) {
      ...
      if (this.transactions[i].transaction.type == 1) {
	    ...
	    //
	    // update staking round if needed
	    //
	    if (this.app.blockchain.current_stakers == 0) {
	      this.app.blockchain.updateStakingRound(this.block.id);
	    }

            //
            // prevblk is not null here
            //
            let payments = prevblk.calculateBlockPayments(this.transactions[i].transaction.msg);
	    ...
	    if (this.containsGoldenTicket() == 1) {
              ...
              if (payments.staker_share > 0) {
                ...
                //
		let winning_slip = this.app.blockchain.index.stakers[payments.staker_pos][payments.staker_sid].slip;
				
console.log("PAYMENTS: " + JSON.stringify(payments));
		let normalized_staker_payout = this.returnNormalizedStakerPayment(winning_slip.amt);
                let blk_st = Big(prevblk.block.st).plus(Big(payments.staker_share)).minus(normalized_staker_payout).toFixed(8);

console.log("_______________________");
console.log("_______ VALIDATE ______");
console.log("_______________________");
console.log("Amount of Staker Payout: " + winning_slip.amt);
console.log("Normalized Staker Payout: " + normalized_staker_payout);
console.log("PREVVLK_ST and StakerShare: " + prevblk.block.st + " -- " + payments.staker_share);

console.log("NORMALIZD STAKER PAYOUT: " + normalized_staker_payout);
console.log("WE THINK BLK_ST IS: " + blk_st + " ( it is ---> " + (this.block.st) + ")");
		...
		this.app.blockchain.current_stakers--;

console.log("VALIDATED NOW SLIP INFO: " + this.app.blockchain.staking_round + " -- " + this.app.blockchain.current_stakers + " -- " + this.app.blockchain.pending_stakers);
console.log("SLIP INDEX NOW: " + JSON.stringify(this.app.blockchain.index.stakers));
	...
	for (let ii = 0; ii < this.transactions[i].transaction.to.length; ii++) {
	  if (this.transactions[i].transaction.to[ii].type == 4) {
	    if (this.transactions[i].transaction.to[ii].isNonZeroAmount()) {
              staking_slips_added++;
    	      staking_amount_added = Big(staking_amount_added).plus(Big(this.transactions[i].transaction.to[ii].amt));
	    }
	  }
	}
      }
	...
	let adjusted_ss = prevblk.block.ss + staking_slips_added - removed_staking_slips;
	let adjusted_sa = Big(prevblk.block.sa).plus(staking_amount_added).minus(removed_staking_amount).toFixed(8);


	//
	// staking data 
	//
	console.log(adjusted_ss + " --SS-- " + this.block.ss); 
	console.log(adjusted_sa + " --SA-- " + this.block.sa); 
...
console.log("\n\nStatus of Block and Treasury: ");
console.log("Mining Treasury: " + this.block.mt);
console.log("Staking Treasury: " + this.block.st);
  ...
  //
  calculateBlockPayments(gt=null) {
    ...
    if (this.block.prevbsh != "") { 
      ...
      prevblk = this.app.blockchain.returnBlockByHash(prevbsh);
    }
    if (prevblk != null) { 
      prevprevbsh = prevblk.block.prevbsh;
      prevprevblk = this.app.blockchain.returnBlockByHash(prevprevbsh);
    }
    ...
    var paysplit_share                  = 0;
    var staker_share                    = 0;
    var srouter_share                   = 0;

    if (prevblk != null) {
      sproducer_share                 = Big(total_work_available_block2).minus(Big(total_work_needed_block2)); // Big
      paysplit_share                  = Big(total_fees_in_block_block2).minus(sproducer_share).plus(prevblk.mc); // Big
      staker_share                    = paysplit_share.div(2).toFixed(8);     // string
      srouter_share                   = paysplit_share.minus(Big(staker_share)).toFixed(8); // string
    }
...
console.log("RETURN WINNING STAKER over");
...
console.log("returning payments from block.js");
...
console.log("Generating Sig: " + this.app.crypto.hash(this.transactions[i].returnSignatureSource(this.app)));
