...
console.log("PAYMENTS WHEN BUNDLING: " + JSON.stringify(payments));
...
console.log("BUNDLING MONETARY POLICY - ss");

            //
            // what is actually paid out?
            //
	    let staker_slip = this.app.blockchain.index.stakers[payments.staker_pos][payments.staker_sid];
console.log("ALL: " + JSON.stringify(this.app.blockchain.index.stakers));
console.log("WINNER AMT: " + staker_slip.slip.amt);
...
console.log("WINNER amount is normalized to: " + normalized_staker_payout + " using st.cb " + this.sc);

            //
            // increment ss and adjust fees into total staked
            //
console.log("incrementing this.block.ss up from: " +this.block.ss + " to ..." + (this.block.ss+1));
	    this.block.ss++;
console.log("incrementing sa from: " + this.block.sa + " to ...");
	    this.block.sa = Big(this.block.sa).plus(normalized_staker_payout).toFixed(8);
console.log(this.block.sa);

	    //
	    // put this block's staking payment into staking treasury
	    //
	    this.block.st = Big(this.block.st).plus(Big(payments.staker_share)).minus(normalized_staker_payout).toFixed(8);

console.log(" blk ====> Block Vars Set for : " + this.block.id);
console.log(" nsp ====> Normalized Staker Payout: " + normalized_staker_payout);
console.log(" ss  ====> " + this.block.ss);
console.log(" sa  ====> " + this.block.sa);
console.log(" st  ====> " + this.block.st);
...
console.log("tx fee pushed into slips!");
...
console.log("\nstaking information blk: " + this.block.id);
console.log("ss: " + this.block.ss);
console.log("sa: " + this.block.sa);

console.log("PAYMENTS WHEN VALIDATING: " + JSON.stringify(payments));
...
console.log("WS: " + JSON.stringify(winning_slip_amt));
...
console.log("WINNER amount is normalized to: " + normalized_staker_payout + " using st.cb " + this.sc);
		...
		//
                ...
                block_st = Big(prevblk.block.st).plus(Big(payments.staker_share)).minus(normalized_staker_payout).toFixed(8);
...
for (let i = 0; i < this.transactions.length; i++) {
  console.log(JSON.stringify(this.transactions[i].transaction.type));
}
...
console.log("removed staking slips? " + eblkd.ss);
...
console.log("twnb1: " + total_work_needed_block1);
      total_work_available_block1	= this.returnEmbeddedRoutingWork();     // string
console.log("twna1: " + total_work_available_block1);

if (parseFloat(total_work_needed_block1) > parseFloat(total_work_available_block1)) {

console.log("\n\nNOT ENOUGH WORK: what is wrong? ");

  for (let z = 0; z < this.transactions.length; z++) {
    console.log(z + " - work: " + this.transactions[z].returnRoutingWorkAvailable(this.app, this.block.creator));
    console.log(z + " from: " + JSON.stringify(this.transactions[z].transaction.from));
    console.log(z + " to: " + JSON.stringify(this.transactions[z].transaction.to));
  }
