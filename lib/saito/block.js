...
console.log("SRouter:   " + payments.srouter_share);
    ...
    if (index_of_golden_ticket > -1) {
	  ...
	  if (slips.length != this.transactions[index_of_block_payment].transaction.to.length) { 
	    console.log("ERROR 582034: golden ticket to slip length inaccurate");
	    return 0; 
	  }
	  for (let z = 0; z < slips.length; z++) {
	    slips[z].bsh = this.transactions[index_of_block_payment].transaction.tp[z].bsh;
	    slips[z].bid = this.transactions[index_of_block_payment].transaction.tp[z].bid;
	    slips[z].tid = this.transactions[index_of_block_payment].transaction.tp[z].tid;
	    slips[z].sid = this.transactions[index_of_block_payment].transaction.tp[z].sid;
