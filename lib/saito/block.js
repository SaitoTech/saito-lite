...
class Block {
    ...
    for (let i = 0; i < this.transactions.length; i++) {
      ...
      if (this.transactions[i].transaction.type == 1) {
        ...
        if (number_of_golden_tickets > 1) {
          logger.info("ERROR 412523: second golden ticket found in block");
          this.is_valid = 0;
          return this;
        }
        ...
        if (prevblk != null) {
          ...
          payments.blocks.forEach((block) => {
            if (block.producer_share > 0) {
              slips.push(new saito.slip(block.producer_publickey, block.producer_share, 1));
            }
            if (block.miner_share > 0) {
              slips.push(new saito.slip(block.miner_publickey, block.miner_share, 1));
            }
            if (block.router_share > 0) {
              slips.push(new saito.slip(block.router_publickey, block.router_share, 1));
            }
            if (block.staker_share > 0) {
              slips.push(new saito.slip(block.staker_publickey, block.staker_share, 1));
            }
          });
            // let {
            //   producer_share,
          ...
          if (this.app.shashmap.validate_slip(slip.returnSignatureSource(), this.block.id)) {
            if (eblk.transactions[i].isRebroadcast(eblk, this, ii)) {
              ...
              if (txs == null) {
                ...
                if (tx == null) {
                  logger.info("ERROR 481029: issue generating rebroadcast transaction...");
                  process.exit();
                }
  ...
  async validate() {
    ...
    try {
      ...
      if (this.block.id != (prevblk.block.id+1)) {
        logger.info("ERROR 482039: block id is not single increment over previous block");
        return 0;
      }
      ...
      if (this.block.treasury != bt) {
        logger.info("ERROR 410829: block treasury does not validate in new block");
        return 0;
      }

      if (this.block.coinbase != cb) {
        logger.info("ERROR 410829: block coinbase does not validate in new block");
        return 0;
      }

      if (this.block.difficulty != prevblk.block.difficulty) {
        logger.info("ERROR 571928: block difficulty does not validate in new block");
        return 0;
      }
      if (this.block.paysplit != prevblk.block.paysplit) {
        logger.info("ERROR 571928: block paysplit does not validate in new block");
        return 0;
      }
      ...
      if (prevblk.block.ts >= this.block.ts) {
        logger.info("ERROR 729384: block timestamp not incremented from previous block");
        return 0;
      }
      ...
      for (let i = this.transactions.length-1; i >= 0; i--) {
        ...
        if (this.transactions[i].transaction.id != (mintxid + i)) {
          logger.info("ERROR 517324: transaction ids are out-of-order when validating block - " + mintxid + "-" + i + " ------- " + this.transactions[i].transaction.id);
          return 0;
        }
        ...
        if (this.transactions[i].transaction.type == 2) {
          if (index_of_block_payment != -1) {
            logger.info("ERROR 581029: two block paymeet (type=2) transactions in block");
            return 0;
          }
        ...
        if (this.transactions[i].transaction.type == 1) {
          if (this.transactions[i].transaction.msg.target != prevblk.returnHash()) {
            ...
            logger.info("ERROR 029312: golden ticket does not match prevblk hash");
            return 0;
            ...
            number_of_golden_tickets++;
            if (number_of_golden_tickets > 1) {
              logger.info("ERROR 189842: two golden tickets in block");
              return 0;
            }

            if (index_of_block_payment == -1) {
              logger.info("ERROR 697812: golden ticket in block, but not block payment transaction");
              return 0;
            }
            ...
            //   if (producer_share > 0) {
            //     slips.push(new saito.slip(producer_publickey, producer_share, 1));
            //   }
            //   if (miner_share > 0) {
            //     slips.push(new saito.slip(miner_publickey, miner_share, 1));
            //   }
            //   if (router_share > 0) {
            //     slips.push(new saito.slip(router_publickey, router_share, 1));
            //   }
            //   if (staker_share > 0) {
            //     slips.push(new saito.slip(staker_publickey, staker_share, 1));
            //   }
            // }
            payments.blocks.forEach((block) => {
              if (block.producer_share > 0) {
            ...
            if (this.app.crypto.hash(JSON.stringify(slips)) == this.app.crypto.hash(JSON.stringify(this.transactions[index_of_block_payment].transaction.to))) {
              ...
              logger.info("ERROR 840192: to slips in block fee transaction invalid");
              logger.info("RECOVERED SLIPS: ", slips);
              logger.info("TX TO SLIPS", this.transactions[index_of_block_payment].transaction.to);
              return 0;
            }
          }
        }
        ...
        for (let j = 0; j < this.transactions[i].transaction.from.length; j++) {
          if (hmap[this.transactions[i].transaction.from[j].returnSignatureSource()] != undefined) {
            logger.info("ERROR 820493: multiple transactions spend same input detected");
            return 0;
          }
        ...
        if (!this.transactions[i].validate(this.app, this)) {
          logger.info(`ERROR 174233: block contains invalid transaction: ${i}`);
      ...
      if (this.app.blockchain.isFullySynced() == 1) {
        ...
        if (!Big(this.block.reclaimed).eq(reclaimed)) {
          logger.info("ERROR 241948: reclaimed funds do not match expected when validating");
          return 0;
        };
        ...
        if (eblkd.validates == false) {
          logger.info("ERROR 115293: rebroadcast transactions do not match when validating");
          return 0;
        };
      }
      ...
      if (tx_sigs.length > 0) {
        if (this.block.merkle != this.app.crypto.returnMerkleTree(tx_sigs).root) {
          logger.info("ERROR 258102: merkle root does not match provided root");
            return 0;
        }
      }
      ...
      logger.info("ERROR 921842: unknown error while validating block...: " + err);
      return 0;
    }
  ...
  returnBlockPayouts(gt=null, rvalue={}, rloop=0) {
    ...
    try {
      ...
      logger.info("ERROR: in checking treasury: ", err);
    }
    ...
    try {
      ...
      logger.info("ERROR IN GOLDEN TICKET CREATION: ", err);
    }
