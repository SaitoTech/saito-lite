...
console.log("DONE CALCULATING NSP");
  ...
  benchmark(msg, oldtime) {
    let time = Date.now();
    let diff = "";
    if (oldtime) {
      diff = time - oldtime;
    }
    console.log("##### HOW FAST ###### " + msg.padEnd(20, ' ') + " " + time + " " + diff);
    return time;
  }
  async validate() {
    this.lastTime = this.benchmark("validate " + this.transactions.length + "tx", this.lastTime);
    ...
    //let timeBefore2 = this.benchmark("!!!!!! starting for loop...");
    for (let i = 0; i <= this.transactions.length-1; i++) {
      //timeBefore2 = this.benchmark(i.toString(), timeBefore2);
	...
	let gtmsg = this.transactions[i].returnMessage();
        ...
        if (gtmsg.target != prevblk.returnHash()) {
          ...
          if (this.app.blockchain.isFullySynced() == 1 && this.app.BROWSER == 0 && this.app.SPVMODE == 0) {
            ...
            //
            console.log("GTMSG: " + JSON.stringify(gtmsg));
...
console.log("Not Fully Synced, so cannot validate block payments, only block hashes...");
