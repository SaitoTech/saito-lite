...
class Block {
  ...
  constructor(app, blkjson = "", txsjson = "", confirmations = -1) {
    ...
    this.block.treasury 	= Big("2868100000.0");  // TODO
                                                        ...
                                                        // fix treasury amount
                                                        //
    this.block.coinbase 	= Big("0.0");		// tokens available to miners if they solve the block reward. this is
							// calculated from the treasury.
						
    this.block.reclaimed 	= Big("0.0");		// tokens that fall off the chain. these are calculated in two places:
							//
							//  - calculateRebroadcast - determines which UTXO are not rebroadcast
							//  - calculateBlockPayments - determines what payments for all 6 recipients

    this.block.st 	      	= Big("0.0"); 		// staking treasury - amount paid into last staking round
    this.block.sr 	      	= Big("0.0"); 		// staking round - which staking round is active ?
    this.block.sp 	      	= Big("0.0"); 		// staking payourt - total to be distributed this round
    this.block.cs 	      	= Big("0.0"); 		// cumulative stake - total staked in active staking round


										     // would be if payments are issued. the validate / bundle
										     // functions need to agree on which are issued and which 
										     // are recollected.

    //
    // aggregate funds staked during the staking round - resets to 0 new round
    //							
    this.funds_staked		= "0";
    this.staker_payout		= "0";
...
console.log("GOLDEN TICKET: " + this.containsGoldenTicket());
    ...
    if (prevblk != null) {
      ...
      if (payments.staker_pos != -1) { 
        ...
        this.app.blockchain.index.stakers[payments.staker_pos][payments.staker_sid].pending = 1; 
      }
	...
	if (this.containsGoldenTicket() == 1) {
          ...
          //
          // reclaim payment to staking treasury -- if it goes into treasury coinbase adjusts
          //
	  this.block.st = Big(this.block.st).plus(Big(payments.staker_share));

	  //
	  // calculate ACTUAL staker payout
	  //
	  let adjusted_staker_payout = payments.staker_share;
	  ...
	  //
	    ...
	    //
            slips.push(new saito.slip(payments.staker_publickey, payments.staker_share, 4));
          }
...
console.log("hash of txs to: " + this.app.crypto.hash(JSON.stringify(this.transactions[index_of_block_payment].transaction.to)));
  ...
  calculateBlockPayments(gt=null) {
    ...
    var paysplit_share                	= Big(total_fees_in_block_block1).minus(producer_share).plus(this.returnCoinbase()); // Big
    ...
    if (prevblk != null) {
      ...
      paysplit_share                  = Big(total_fees_in_block_block2).minus(sproducer_share).plus(prevblk.returnCoinbase()); // Big
    ...
    // pick the staker
    //
    let x = this.app.blockchain.returnWinningStaker(gt, winnerHash3);
    ...
    payments.sproducer_share	= sproducer_share;
