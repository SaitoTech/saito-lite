...
class SaitoHeader extends UIModTemplate {
  ...
  render(app, mod) {
    ...
    this.cryptoMod = app.wallet.returnPreferredCryptoModule();
    this.cryptoMod.eventEmitter.on("balance_change", this.balanceChangeListener);
    app.wallet.onPreferredCryptoChangeEvent(() => {
      // remove balance change listener for old crypto, add listener for new crypto
      this.cryptoMod.eventEmitter.removeListener("balance_change", this.balanceChangeListener);
      this.cryptoMod = app.wallet.returnPreferredCryptoModule();
      this.cryptoMod.eventEmitter.on("balance_change", () => {
        this.renderBalance();
      });
  ...
  async renderBalance(app, mod) {
    document.getElementById("header-token").innerHTML = " " + this.cryptoMod.ticker;
    document.getElementById("header-balance").innerHTML = "loading...";
    document.getElementById("header-balance").innerHTML = await this.cryptoMod.getBalance();
  }
