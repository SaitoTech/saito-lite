...
class SaitoHeader extends UIModTemplate {
  ...
  render(app, mod) {
    ...
    app.connection.on("set_preferred_crypto", (data) => { this.renderCrypto(app, mod); }
  ...
  async renderCrypto() {
    ...
    //
    // available cryptos are modules that support is_cryptocurrency
    //
    ...
    // set default crypto
    //
    document.getElementById("profile-public-key").innerHTML = await this.app.wallet.returnPreferredCryptoAddress();
    ...
    app.browser.addElementToElement(add_other_cryptos_html, document.querySelector("#header-token-select"));
