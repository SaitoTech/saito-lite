...
class SaitoHeader extends UIModTemplate {
  ...
  async renderCrypto() {
    ...
    // set default crypto
    //
    document.getElementById("profile-public-key").innerHTML = await this.app.wallet.returnPreferredCrypto().getAddress();
  ...
  attachEvents(app, mod) {
    ...
    document.querySelectorAll('#header-logo-link').forEach((element) => {
      element.onclick = (event) => {
        ...
        if(activeModule) {
          app.browser.logMatomoEvent("Navigation", `${activeModule.name}NavigationClick`, "HeaderLogoHomepageLink");  
        } else {
          app.browser.logMatomoEvent("Navigation", `UnknownModuleNavigationClick`, "HeaderLogoHomepageLink");  
