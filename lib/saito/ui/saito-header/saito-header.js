...
class SaitoHeader extends UIModTemplate {
  ...
  async renderCrypto() {
    try {
      ...
      console.log("renderCrypto", document.querySelector("#header-token-select").innerHTML);
      
      let html = `<option id="crypto-option-${preferred_crypto.name}" value="${preferred_crypto.ticker}">Loading...</option>`;
      ...
      for (let i = 0; i < available_cryptos.length; i++) {
        ...
        if (crypto_mod.name != preferred_crypto.name) {
          let html = `<option id="crypto-option-${crypto_mod.name}" value="${crypto_mod.ticker}">Loading...</option>`;
          this.app.browser.addElementToElement(html, document.querySelector("#header-token-select"));
      ...
      // console.log("renderCrypto2", document.querySelector("#header-token-select").innerHTML);
