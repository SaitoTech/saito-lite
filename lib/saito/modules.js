class Mods {
  ...
  handlePeerRequest(message, peer, mycallback=null) {
    for (let iii = 0; iii < this.mods.length; iii++) {
      try {
console.log("pre handle peer request...");
...
//
for (let i = 0; i < this.mods.length; i++) {
  console.log("exists: " + this.mods[i].name);
}
console.log(JSON.stringify(this.app.options.modules));
...
console.log("splicing out: " + this.mods[z].name);
...
console.log("pre install new modules...");
...
console.log("MODULES: " + this.app.options.modules.length + " --- " + this.mods.length);
      if (this.app.options.modules.length < this.mods.length) { start_installation = 1; }
    }
    if (start_installation) {
      for (let i = 0; i < this.mods.length; i++) {
console.log("examining : " + this.mods[i].name);
        let mi = 0;
        let mi_idx = -1;
...
console.log("should we install? " + install_this_module);

	if (install_this_module == 1) {
          new_mods_installed = 1;
console.log("INSTALLING: " + this.mods[i].name);
...
console.log("INSTALLED!");
    ...
    //
    for (let i = 0; i < this.mods.length; i++) {
console.log("initializing mod: " + i + " -- " + this.mods[i].name);
      this.mods[i].initialize(this.app);
    }


console.log("attaching events 14132...");
...
console.log("attaching events 14132 2...");

    //
    // .. and setup active module
    //
    await this.app.modules.initializeHTML();
console.log("attaching events 14132 3...");
    await this.app.modules.attachEvents();
console.log("attaching events 14132 4...");
