...
class Mempool {
  ...
  canBundleBlock() {
    ...
    logger.info("Total Work Needed: " + this.routing_work_needed + " ---- available ---> " + this.routing_work_in_mempool + "     (" + this.app.wallet.returnBalance() + ")");

    if (this.routing_work_in_mempool >= this.routing_work_needed) { 
      logger.info(" ... allowing block production ");
      return 1;
    }
    return 0;
    ...
    for (var key in this.downloads) {
      while (this.downloads[key].length > 0) {
        ...
        if (peer != null) {
          ...
          logger.info("Couldn't find peer by key, attemping to re-establish peer connections");
    ...
    // if ( this.block_size_current <= this.block_size_cap ) {
      ...
      try {
        ...
        if (blk.block.ts > new Date().getTime() + 60000) {
          logger.info("block appears to be from the future, dropping...");
          return;
        }
  ...
  addBlock(blk) {
    ...
    if ( (this.block_size_current + blk.size) > this.block_size_cap) {
      logger.info("ERROR 629384: mempool block queue at size limit");
      return;
    }
...
logger.info("removing golden ticket!");
      ...
      if (blk.transactions.length == 0 && blk.block.id > 1) {
        ...
        logger.info("ERROR 51958: produced block with zero transactions. Aborting.");
...
*****/
      ...
      if (blk.is_valid == 0) {
        logger.info("ERROR 105812: block invalid when bundling. Aborting block bundling...");
        this.bundling_active = false;
        return;
      }
      ...
      logger.info("ERROR 781029: unexpected problem bundling block in mempool: " + err);
    }
  ...
  removeBlockAndTransactions(blk=null) {
    ...
    for (let t = 0; t < this.transactions.length; t++) {
      if (this.transactions[t].may_be_removed_from_mempool != 1) {
        ...
        //logger.info("TX DROPPED");
        //logger.info(this.transactions[t]);
      }
    }
