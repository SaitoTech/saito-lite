...
class Mempool {
  ...
  canBundleBlock() {
    ...
    if (this.app.network.downloading_active == 1)			{ return 0; }
...
console.log("we have added a block: " + blk.returnHash());
...
console.log("processing block queue 1...");
    if (this.processing_active == 1) { return; }
    this.processing_active = 1;
console.log("processing block queue 2...");
    ...
    try {
console.log("trying to process blocks....");
      this.processing_timer = setInterval(async () => {
console.log("do we have blocks? " + this.blocks.length);
        if (this.blocks.length > 0) {
console.log("processing block ... indexing active: " + this.app.blockchain.indexing_active);
          if (this.app.blockchain.indexing_active == 0) {
            let blk = this.blocks.shift();
            await this.app.blockchain.addBlockToBlockchain(blk);
          }
        } else {
console.log("not processing and clearing our processing interval.");
...
console.log(" and out of process block queue...");
