...
class Mempool {
        ...
        if (peer != null) {
          ...
          if (block_hashes.length > 1) {
            try {
              ...
              console.log("ERROR 15445: In fetchBlocks");
              console.log(err);
            }
          } else {
            try {
              ...
              console.log("ERROR 15446: In fetchBlocks");
              console.log(err);
            }
          }
        } else {
  ...
  async fetchSingleBlock(block_to_download_url, bhash) {
    return new Promise((resolve, reject) => {
      ...
      //     // we need to add this chunk to our blockObj
      //     // if (errorMsg == "parseerror") {
      //     //   blockObjs[blockObjs.length - 1] +=
      //     // }
      //     reject(errorMsg);
      //   },
      //   "complete": (statusText) => {
      ...
      // jsonpipe.flow(block_to_download_url, options);
      ...
      xhr.open('GET', block_to_download_url);
      xhr.onreadystatechange = () => {
        switch (xhr.readyState) {
          case state.LOADING:
            if (xhr.responseText) body = xhr.responseText;
            break;
          case state.DONE:
            if (xhr.status == 200) {
              body = body.split('^\n')[0];
              if (body) {
                ...
                let blk = new saito.block(this.app, bodyObj);
                if (blk.block.ts > new Date().getTime() + 60000) {
                  console.log("block appears to be from the future, dropping...");
                  return;
                }
                if (blk.is_valid == 0 && this.app.BROWSER == 0) return;
                this.addBlock(blk)
                delete this.downloads_hmap[bhash];
              }
              resolve();
  ...
  async fetchMultipleBlocks(block_to_download_url, blocks, publickeys=[]) {
    return new Promise((resolve, reject) => {
      try {
        ...
        var data = this.app.crypto.stringToBase64(JSON.stringify({
          blocks,
          publickeys
        }));
        ...
        let options = {
          ...
          "complete": (statusText) => {
            ...
            blockObjs.sort((a, b) => parseInt(a.block.id) - parseInt(b.block.id));
            console.log(blockObjs.map(blk => blk.block.id));
            blockObjs.forEach((body, index) => {
              let blk = new saito.block(this.app, body);
