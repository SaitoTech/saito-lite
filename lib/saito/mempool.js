...
console.log("REJECTING TX as its SIG in HMAP: " + tx.transaction.sig);
console.log("REJECTIED TX MSG is: " + tx.transaction.msg);
      return 1; 
    }
...
console.log("REJECTING TX as SLIP_IDX in HMAP: " + slip_index);
console.log("EXISTING IS: " + this.transactions_inputs_hmap[slip_index]);
console.log("MEMPOOL IS: " + JSON.stringify(this.transactions));
          return 1;
        }
      } else {
      }
  ...
  async bundleBlock() {
	...
	let removed_tx = 0;
	    ...
	    removed_tx = 1;
          ...
          if (Math.random() < 0.5) {
console.log("REMOVING GT for STAKING NEEDS");
            this.transactions.splice(i, 1);
	    removed_tx = 1;
            addtx = 0;
          }
...
console.log("transactions b4? " + this.transactions.length);
console.log("blk txs: " + blk.transactions.length);

    //
    // create hashmap for mempool transactions
    //
    for (let b = 0; b < this.transactions.length; b++) {
console.log("mempool contains: " + this.transactions[b].transaction.sig);
...
console.log("found one from block: " + blk.transactions[b].transaction.sig);
...
console.log("transactions left? " + this.transactions.length);
