...
class Server {
  ...
  initialize() {
    ...
    if (this.server.host == "" || this.server.port == 0) {
      logger.info("Not starting local server as no hostname / port in options file");
      return;
    }
  ...
  // const io          = require('socket.io')(webserver);
    ...
    //         }
    //       } catch(err) {
    //         logger.error("FETCH BLOCKS ERROR: ", err);
    //         res.status(400)
    //         res.send({
    //           error: {
    ...
    //     }
    //   } catch (err) {
    //     logger.error("FETCH BLOCKS ERROR SINGLE BLOCK FETCH: ", err);
    //     res.status(400)
    //     res.send({
    //       error: {
  ...
  /*****
    ...
    app.post('/lite-blocks', async (req, res) => {
      ...
      try {
        ...
        logger.error("FETCH LITE BLOCKS ERROR: ", err);
        res.status(400)
        res.send({
          error: {
    ...
    app.get('/lite-blocks/:bhash/:pkey', async (req, res) => {
      ...
      try {
        ...
        logger.error(`FAILED SERVER REQUEST: could not find block: ${bhash}`);
        logger.error("FETCH LITE BLOCKS ERROR: ", err);
        res.status(400)
        res.send({
          error: {
