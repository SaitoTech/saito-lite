...
class Keychain {
  ...
  fetchIdentifier(publickey = "", mycallback) {
    ...
    this.modtemplate.sendPeerDatabaseRequest("registry", "records", "*", "", null, (res) => {
      ...
      if (res.rows.length == 0) { callback(rows); }
  ...
  fetchManyIdentifiers(publickeys = [], callback) {
    //DL-CHECK
    //Function now calls sendPeerDatabaseRequestWithFilter
    let found_keys = {};
    let missing_keys = [];
    ...
    // updated db call - included a filter for determining the peer to requrest from.
    this.modtemplate.sendPeerDatabaseRequestWithFilter("registry", sql, null, (peers) => {
      //filter callback to only bother peers that claim they support DNS
      if (this.app.network.peers.length > 1) {
        return peers.filter(function (obj, index, arr) {return obj.peer.service =="dns"})
      }
    }, (res) => {
      let rows = [];
      if (res.rows == undefined) { callback(rows); }
      if (res.err) { callback(rows) }
      if (res.rows == undefined) { mycallback(rows); }
      if (res.rows.length == 0) { callback(rows); }
  ...
  fetchManyIdentifiers(publickeys = [], callback) {
    ...
    // add sendPeerDatabaseRequestWithFilter
