...
***REMOVED***);
    ...
    if (missing_keys.length == 0) return found_keys;
    let where_statement = `publickey in (${missing_keys.join(',')***REMOVED***`;

    this.modtemplate.sendPeerDatabaseRequest("registry", "records", "*", where_statement, null, function(res) {
      let rows = [];
      if (res.rows == undefined) callback(rows);
      rows = rows.map(row => {
        let row_obj = JSON.parse(row);
        let { publickey, identifer ***REMOVED*** = row_obj;
        found_keys[publickey] = identifer;
  ***REMOVED***);
      callback(found_keys);
***REMOVED***);
  ***REMOVED***
