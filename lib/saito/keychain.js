...
console.log("add key: " + publickey);

    let tmpkey = this.findByPublicKey(publickey);
    if (tmpkey == null) {
console.log("adding as new key");
      ...
      tmpkey.lc = lc;
      tmpkey.name = "";
      if (identifier != "") { tmpkey.addIdentifier(identifier); tmpkey.name = identifier; }
      if (tag != "") { tmpkey.addTag(tag); }
      this.keys.push(tmpkey);
    } else {
console.log("updating: " + bsh + " -- " + bid);
console.log("existing: " + tmpkey.bsh + " -- " + tmpkey.bid);
      if (bid != "" && bsh != "") {

        if (tmpkey.bsh != bsh && tmpkey.bid != bid) {

console.log("UPDATING INFO 1");
/*** TODO -- it is possible to have outdated information with wallet resets
 * so we do not want to repeatedly save, so we don't save at all in this 
 * case. You cannot add the same key twice this way.
          tmpkey = new saito.key();
          tmpkey.publickey = publickey;
          tmpkey.watched = watched;
          tmpkey.bid = bid;
