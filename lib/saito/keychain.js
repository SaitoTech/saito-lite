...
class Keychain {
  ...
  addWatchedPublicKey(publickey="") {
    ...
    logger.info("FUNCTIONALITY NOT IMPLEMENTED -- addWatchedPublicKey");
    process.exit(1);
  ...
  clean() {
    for (let x = 0; x < this.keys.length; x++) {
      if (this.keys[x].isWatched() == false) {
        if (this.keys[x].aes_secret != "") {
  	  logger.info("purging key records: " + this.keys[x].publickey + " " + JSON.stringify(this.keys[x].identifiers));
          this.keys.splice(x, 1);
	  x--;
        }
      }
