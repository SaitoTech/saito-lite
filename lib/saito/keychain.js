...
console.log("ENTERING DECRYPT MESSAGE");

    // submit JSON parsed object after unencryption
    for (let x = 0; x < this.keys.length; x++) {
console.log("CHECKING " + this.keys.length);
      if (this.keys[x].publickey == publickey) {
console.log("found our partner");
        if (this.keys[x].aes_secret != "") {
console.log("decrypting encrypted msg: " + JSON.stringify(encrypted_msg));
          var tmpmsg = this.app.crypto.aesDecrypt(encrypted_msg, this.keys[x].aes_secret);
console.log("jsontmpmsg: " + JSON.stringify(tmpmsg));
          if (tmpmsg != null) {
console.log("tmpmsg: " + tmpmsg);
            var tmpx = JSON.parse(tmpmsg);
console.log("tmpmsg2: " + JSON.stringify(tmpx));
            if (tmpx.module != null) {
console.log("tmpmsg2: " + JSON.stringify(tmpx));
              return tmpx;
            }
