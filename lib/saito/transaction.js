...
console.log("\t\t\tREBROADCASTING A VIP TX: " + JSON.stringify(this.transaction));
      return true; 
    }
  ...
  validate(app, bid=0) {
	...
	//
        if (!app.shashmap.validate_slip(this.transaction.from[i].returnSignatureSource(), bid)) {
          console.log("ERROR 570030: cannot validate inputs with shashmap...");
          this.is_valid = 0;
          return false;
	}
      }
    ...
    if (this.transaction.type == 1) { 
      ...
      if (!app.crypto.verifyMessage(this.returnSignatureSource(app), this.transaction.sig, this.transaction.from[0].add)) {
        console.log("ERROR 137102: signature on golden ticket does not validate");
        return 0;
      }
    ...
    if (this.transaction.type == 2) { 
      ...
      if (!app.crypto.verifyMessage(this.returnSignatureSource(app), this.transaction.sig, this.transaction.from[0].add)) {
        console.log("ERROR 137102: signature on fee transaction does not validate");
        return 0;
      }
