...
class Transaction {
  ...
  constructor(txobj=null) {
    ...
    this.transaction.r             = 1; // "replaces" (how many txs this represents in merkle-tree -- spv block)
	...
	//
	// catch errors - txobj should not have transaction field
	//
        if (txobj.transaction) { this.transaction = txobj.transaction; }
        ...
        this.msg = JSON.parse(this.base64ToString(this.transaction.m));
    ...
    this.size = JSON.stringify(this.transaction).length;
  ...
  generateRebroadcastTransaction(creator, tid, sid, avg_fee=2) {
    ...
    if (
      ...
      //
      if (this.msg.loop == undefined) {
        newtx.msg.loop = 1;
      } else {
        newtx.msg.loop = this.msg.loop+1;
      }
      ...
      //
      if (this.msg.tx != undefined) {
        newtx.msg.tx = this.msg.tx;
      } else {
        newtx.msg.tx = JSON.stringify(this.transaction);
      }
...
console.log("decryotung tx from other");
console.log("ORIG: " + JSON.stringify(this.msg));
      try {
        //let parsed_msg = JSON.parse(this.msg);
        let parsed_msg = this.msg;
console.log("parsed: " + JSON.stringify(parsed_msg));
        this.dmsg = app.keys.decryptMessage(this.transaction.from[0].add, parsed_msg);
console.log("done parsing");
      } catch (e) {
	console.log("ERROR: " + e);
      }
console.log("DECRYPTED: " + JSON.stringify(this.dmsg));
      return;
    }
console.log("decryotung tx from me");
  ...
  returnSignatureSource(app) {
    ...
    for (let i = 0; i < this.transaction.from.length; i++) {
      ss += this.transaction.from[i].returnSignatureSource();
    }
    for (let i = 0; i < this.transaction.to.length; i++) {
      ss += this.transaction.to[i].returnOriginalSignatureSource(); // bid, sid, tid as zero
    }

    ss += `${this.transaction.ts}${this.transaction.type}${this.transaction.r}${this.transaction.m}${this.transaction.ps}`;
    return ss;
...
console.log("_____________________________________________");
console.log("_____________________________________________");
console.log("This transaction contains pending staking TXS");
      return true;
    }


    //
    // SPV transactions validate as long as they don't do anything
    //
    if (this.transaction.type == 9) {
      if (this.transaction.from.length > 0) { return false; }
      if (this.transaction.to.length > 0) { return false; }
      return true;
    }
...
console.log("BSH of UTXO spent: " + this.transaction.from[i].bsh);
              console.log("ERROR 185013: transaction tries to spend input off longest chain...");
	      console.log("trying to spend from: " + this.transaction.from[i].bsh + " ---->\n TX: " + JSON.stringify(this.transaction));
  	      this.is_valid = 0;
              return false;
            }
          }
...
console.log(this.returnSignatureSource(app));
console.log(this.transaction.sig);
console.log(this.transaction.from[0].add);
