...
class Transaction {
  ...
  returnSignatureSource(app) {
    ...
    //
    // Browser Internals are UTF16
    //
    if (app.BROWSER == 1) {

      ss += `${this.transaction.ts}${this.transaction.type}${this.transaction.r}${Buffer.from(JSON.stringify(this.transaction.msg), utf16le').toString('utf8')}${this.transaction.ps}`;
      ...
      // non-browsers are ok
      //
      ss += `${this.transaction.ts}${this.transaction.type}${this.transaction.r}${JSON.stringify(this.transaction.msg)}${this.transaction.ps}`;
