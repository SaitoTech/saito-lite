...
class Transaction {
  ...
  constructor(txobj=null) {
    ...
    this.transaction.r             = 1;   // how many rows in merkle tree? -- used for SPV
...
console.log("CREATING TX FROM: " + JSON.stringify(txobj));
  ...
  // TODO
  //
  // this has to be the whole unique series of data associated with a tx, not just thre
  //
  returnSignatureSource(app) {
    return `
      ${JSON.stringify(this.transaction.from)}
      ${JSON.stringify(this.transaction.to)}
      ${this.transaction.ts}
      ${this.transaction.ver}
      ${this.transaction.type}
      ${JSON.stringify(this.transaction.msg)}
      ${this.transaction.ps}
    `;
  }
