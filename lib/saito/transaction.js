...
class Transaction {
  ...
  constructor(txobj=null) {
    ...
    this.transaction.type          = 0;
	    ...
	    // 8 = golden chunk -- deprecated
	    //
	    // 9 = spv lite tx -- replaces transactions in lite blocks
  ...
  returnPayment(app) {
    ...
    let outputs = Big(0.0);
    for (let v = 0; v < this.transaction.to.length; v++) {
      // only count payments that come back
      if (this.transaction.to[v].type === app.wallet.returnPublicKey()) {
        outputs = outputs.plus(Big(this.transaction.to[v].amt));
      }
    }

    let amount_spent = inputs.minus(outputs);
    return amount_spent.toFixed(8);
  ...
  validate(app, bid=0) {
    ...
    if (this.is_valid == 0) {
      console.log("ERROR 329615: transaction is already marked invalid");
      return false;
    }
