...
class Network {
  ...
  initialize() {
    ...
    setInterval(() => { this.pollPeers(this.app.network.peers) }, 15000);

  }

  pollPeers(peers) {
    console.log("total peers: " + peers.length);
    console.log('dead peers: ' + this.dead_peers.length);
    //check if any peers are not connected
    //rely on logic in cleanupDisconnectedSocket to
    //determine if peer should be dropped or added to 
    //list of peers to reconnect (dead_peers)
    peers.forEach((peer) => {
      console.log("checking peer health");
      console.log("peer host: " + peer.peer.host);
      console.log("peer endpoint: " + peer.peer.endpoint.host);
      console.log("peer connected: " + peer.socket.connected);
    ...
    console.log('dead peers: ' + this.dead_peers.length);
    //just try to create peers, if they don't connect they will
    //be readded by pollPeers()
    this.dead_peers.forEach((peer) => {
      this.addPeer(JSON.stringify(peer));
    });
    this.dead_peers = [];
  }
  ...
  cleanupDisconnectedSocket(peer) {
    ...
    for (let c = 0; c < this.peers.length; c++) {
      if (this.peers[c] == peer) {
        console.log('checking peer for removal');
        var keep_peer = -1;
        ...
        //
        // do not remove peers if it's end point is ni our options
        //
        ...
        console.log('removing peer: ' + peer.peer.endpoint.host);
        //
  ...
  async fetchBlocks() {
    ...
    for (var key in this.downloads) {
      console.log("BLOCKS TO DOWNLOAD FROM " + key + ": " + this.downloads[key].length);
      while (this.downloads[key].length > 0) {
        for (let d = 0; d < this.downloads[key].length && d != -1;) {

          console.log("DOWNLOADS LEFT: " + this.downloads[key].length);
          ...
          if (peer != null) {
            ...
            console.log("PEER DOWNLOAD URL: " + block_to_download_url);

            let x = null;

            try {

              x = await this.fetchBlock(`${block_to_download_url}/${this.downloads[key][d]}/${mypkey}`, this.downloads[key][d]);
  ...
  async fetchBlock(block_to_download_url, bhash) {
    ...
    console.log("fetch: " + block_to_download_url + " ----> " + bhash);

    return new Promise((resolve, reject) => {
      fetch(block_to_download_url)
        .then(response => {

          console.log("\n\nfetched: " + block_to_download_url);
          console.log("\n");
          ...
          //
          // the block format has changed -- UPDATE REQUIRED -- 
          //
          response.text().then(data => {
            ...
            console.log("adding the block to the mempool!");
            this.app.mempool.addBlock(blk);
            console.log("deleting the hash 1: " + bhash);
            console.log("deleting the hash 2: " + blk.returnHash());
