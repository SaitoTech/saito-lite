...
console.log("ADDING PEER: " + JSON.stringify(this.app.options.peers[i]));
        this.addPeer(JSON.stringify(this.app.options.peers[i]), 0);
      }
    }

    if (this.app.BROWSER == 1) {
      if (this.app.options.services != null) {
        for (let i = 0; i < this.app.options.services.length; i++) {
          console.log("Initialize our Network and Connect to Service Peers");
  console.log("ADDING PEER: " + JSON.stringify(this.app.options.peers[i]));
          this.addPeer(JSON.stringify(this.app.options.services[i]), 0, 0, 0);
        }
      }
    }
...
console.log("\n\n\nADDING PEER: ");
console.log(JSON.stringify(peerobj));
...
console.log("already in our peer list, so exiting...");
        return;
      }
    }
...
console.log("PEER DOWNLOAD URL: " + block_to_download_url);
	    ...
	    try {
              ...
              x = await this.fetchBlock(`${block_to_download_url}/${this.downloads[key][d]}/${mypkey}`);
...
console.log("fetch: " + block_to_download_url);
