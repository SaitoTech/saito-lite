...
class Network {
  ...
  async wsConnectToRustPeer() {
    ...
    ws.on('open', (event) => {
      ...
      const text = enc.encode("SHAKINIT");
      const publickey_uint_array = new Uint8Array(
        Buffer.from(Base58.decode(this.app.wallet.returnPublicKey()), 'Uint8Array')
        //Buffer.from(this.app.crypto.uncompressPublicKey(this.app.wallet.returnPublicKey()))
      );
      console.log("publickey_uint_array");
      console.log(publickey_uint_array.length);
      const array = new Uint8Array([
        ...text,
        0,0,0,0,
        127,0,0,1,
        ...publickey_uint_array
      ]);
      // array.set();
      // array.push(0)
      // array.push(127)
    ...
    ws.on('message', (data) => {
      ...
      if (`${data.slice(0,8)}` == "RESULT__") {

      }
      let handshake_challenge = this.deserializeHandshakeChallenge(data.slice(12));
      console.log(handshake_challenge.their_ip_octets);
      console.log(handshake_challenge.my_ip_octets);
      console.log(handshake_challenge.their_address);
      console.log(handshake_challenge.my_address);
      console.log(handshake_challenge.timestamp);
      console.log(handshake_challenge.their_sig);
      ...
      console.log(handshake_challenge.their_sig);
      let base58_their_sig = this.app.crypto.toBase58(handshake_challenge.their_sig);
      console.log(this.app.crypto.toBase58(handshake_challenge.their_sig));

      // let is_sig_valid = secp256k1.verify(
      //   Buffer.from(blake3.hash(Buffer.from(buffer.slice(0, 82),'hex')),'hex'),
      //   Buffer.from(this.app.crypto.fromBase58(base58_their_sig),'hex'),
      //   Buffer.from(this.app.crypto.uncompressPublicKey(this.app.crypto.compressPublicKey(handshake_challenge.their_address)),'hex')
      // );
  ...
  async connectToRustPeer() {
    const fetch = require('node-fetch');
    console.log(fetch);
    console.log(this.app.wallet.returnPublicKey());

    try {
      const url = `http://127.0.0.1:3030/handshakeinit?a=${this.app.crypto.uncompressPublicKey(this.app.wallet.returnPublicKey())}`;
      ...
      console.log(`${their_ip_octets[0]}.${their_ip_octets[1]}.${their_ip_octets[2]}.${their_ip_octets[3]}`);
      console.log(`${my_ip_octets[0]}.${my_ip_octets[1]}.${my_ip_octets[2]}.${my_ip_octets[3]}`);
      console.log(this.app.crypto.compressPublicKey(their_address));
      // console.log(this.app.crypto.compressPublicKey(my_address));

      console.log(their_sig);
      let base58_their_sig = this.app.crypto.toBase58(their_sig);
      console.log(this.app.crypto.toBase58(their_sig));

      let is_sig_valid = secp256k1.verify(
        Buffer.from(blake3.hash(Buffer.from(buffer.slice(0, 82),'hex')),'hex'),
        Buffer.from(this.app.crypto.fromBase58(base58_their_sig),'hex'),
        Buffer.from(this.app.crypto.uncompressPublicKey(this.app.crypto.compressPublicKey(their_address)),'hex')
      );
      console.log(is_sig_valid);

      let hex_private_key = this.app.crypto.generateKeys();
      let base58_public_key = this.app.crypto.returnPublicKey(hex_private_key);
      let base58_sig = this.app.crypto.signMessage("asdf", hex_private_key);
      let is_valid = this.app.crypto.verifyMessage("asdf", base58_sig, base58_public_key);

      console.log(hex_private_key);
      console.log(base58_public_key);
      console.log(base58_sig);
      console.log(is_valid);

      let { signature, recovery } = secp256k1.sign(
        Buffer.from(blake3.hash(buffer), 'hex'),
      ...
      const handshake_complete_url = `http://127.0.0.1:3030/handshakecomplete`;
      const res_complete = await fetch(handshake_complete_url, { method: 'POST', body: bytes });
    ...
    // fetch('http://127.0.0.1:3030/handshakeinit?a=' + this.app.wallet.returnPublicKey())
    //     .then(res => {
    //         console.log(res.ok);
    //         console.log(res.status);
    //         console.log(res.statusText);
