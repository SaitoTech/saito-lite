...
console.log("BLOCK HASHES TO DOWNLOAD: " + JSON.stringify(block_hashes));
	  ...
	  let blocks_to_download = block_hashes.length;

	  for (let z = 0; z < blocks_to_download; z++) {
	    if (peer.peer.synctype == "lite") {
console.log("downloading: " + block_hashes[z]);
              await this.fetchBlock(`${block_to_download_url}/${block_hashes[z]}/${mypkey}`);
	    } else {
console.log("downloading: " + block_hashes[z]);
              await this.fetchBlock(`${block_to_download_url}/${block_hashes[z]}/${mypkey}`);
	    }
	    blocks_to_download = block_hashes.length;
	  }
