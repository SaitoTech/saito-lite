...
class Network {
  ...
  constructor(app) {
    ...
    this.peer_monitor_timer_speed = 20000;  // check socket status every 20 seconds
  ...
  initialize() {
    ...
    this.peer_monitor_timer = setInterval(() => { this.pollPeers(this.app.network.peers) }, this.peer_monitor_timer_speed);
  ...
  pollPeers(peers) {
    ...
    this.dead_peers.forEach((peer) => {
      ...
      //setTimeout(() => {this_network.addPeer(JSON.stringify(peer))}, this.peer_monitor_timer_speed - this.peer_monitor_connection_timeout);
      this_network.addPeer(JSON.stringify(peer));
    });
    this.dead_peers = [];
  }
