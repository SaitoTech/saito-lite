...
console.log("RECTX: " + this.receivetxs);
...
console.log("SENDING TX TO PEER? " + peer.peer.receivetxs);

      if (peer.peer.receivetxs == 0) { return; }
      if (!peer.inTransactionPath(tx) && peer.returnPublicKey() != null) {
        let tmptx = peer.addPathToTransaction(tx);
        if (callback) {
  ...
  propagateTransactionWithCallback(tx, mycallback=null) {
    if (peer.peer.receivetxs == 0) { mycallback({ "err" : "peer does not receive transactions" }); }
