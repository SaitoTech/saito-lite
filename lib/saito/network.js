...
class Network {
  ...
  deserializeTransaction(buffer, start_index, inputs_len, outputs_len, message_len, path_len) {
    let si = start_index;

    let signature = this.app.crypto.stringToHex(buffer.slice(si + 16, si + 80));
    let timestamp = this.u64FromBytes(buffer.slice(si + 80, si + 88));
  ...
  deserializeSlip(buffer) {
    ...
    let slip_ordinal = buffer[73];
    let slip_type = buffer[SLIP_SIZE - 1];
