...
class Browser {
  ...
  addDragAndDropFileUploadToElement(id, handleFileDrop) {
  ...
  makeDraggable(id_to_move, id_to_drag="", mycallback=null) {
    try {
      ...
      let element_updated_left = 0;
      let element_updated_top = 0;
	...
	mouse_current_top = mouse_down_top;
	  ...
	  let adjustmentX = mouse_down_left - mouse_current_left;
	  let adjustmentY = mouse_down_top - mouse_current_top;
...
console.log("error: " + err);
  ...
  modifyHash(oldHash, newHashStuff) {
    return this.buildHash(Object.assign(this.parseHash(oldHash), newHashStuff));
  }
  // TODO:
  getValueFromHashAsBoolean(){
    
  }
  getValueFromHashAsNumber(){
