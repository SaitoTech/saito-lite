...
class Browser {
  ...
  async initialize(app) {
    ...
    try {
      ...
      for (pair of entries) {
        console.log("URL PARAMS: " + pair[0] + pair[1]);
      }
    ...
    if (this.app.BROWSER == 1) {
      ...
      this.app.connection.on('connection_up', function (e) {
        siteMessage('Websocket Connection Established', 1000);
      });

      this.app.connection.on('connection_down', function (e) {
  ...
  addDragAndDropFileUploadToElement(id, handleFileDrop=null, click_to_upload=true) {
    ...
    let hidden_upload_form = `
      <form class="my-form" style="display:none">
        ...
        <input type="file" id="hidden_file_element_${id}" multiple accept="image/*">
  ...
  makeDraggable(id_to_move, id_to_drag = "", mycallback = null) {
    ...
    try {
      ...
      var element_updated_left = 0;
      var element_updated_top = 0;

console.log("onmouse down even for: " + element_to_drag.id);
...
console.log("down 1");
        ...
        element_updated_left = element_updated_left;
        element_updated_top = element_start_top;
console.log("down 1");
...
console.log("mdl: " + mouse_down_left);

          mouse_current_left = e.clientX;
console.log("cur left: " + mouse_current_left);
...
console.log("adjustment -- " + adjustmentX);
console.log("esl: " + element_start_left);

          if (adjustmentX > 0) { element_moved = 1; }
          if (adjustmentY > 0) { element_moved = 1; }

          // set the element's new position:
          element_to_move.style.left = (element_start_left + adjustmentX);
          //element_to_move.style.left = (element_start_left + adjustmentX) + "px";
console.log("updated left to: " + element_to_move.style.left);
...
console.log("draggable - on mouse up");
          document.onmouseup = null;
          document.onmousemove = null;
          if (mycallback != null) { if (element_moved == 1) { mycallback(); } }
        };
...
console.log("TOUCH mcl: " + mouse_current_left + " -- " + mouse_current_top + " --> " + adjustmentX);
