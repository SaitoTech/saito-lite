...
class Browser {
  ...
  constructor(app) {
    ...
    //
    this.active_tab = 1; 
    this.active_tab_timer = null;
    this.active_tab_speed = 5000;
  }
...
console.log("creating Saito intertab channel...");
        const channel = new BroadcastChannel('saito');

        channel.onmessage = function(e) {
          if (!document.hidden) {
            console.log('Received while active, updating to active tab', e.data);
            channel.postMessage({ active : 1 });
            this.active_tab = 1;
          } else {
            console.log('Received while hidden', e.data);
            this.active_tab = 0;
          }
        };

	//
	// set timer for periodic visibility checks
	//
        this.active_tab_timer = window.setInterval(() => {
	  if (!document.hidden) {
	    //
	    // if we are visible, take over the active tab
	    //
	    this.active_tab = 1;
            channel.postMessage({ active : 1 });
	  } else {
	    //
	    // if we are not visible, announce so others can take-over if they are visible
	    //
            channel.postMessage({ active : 0 });
	  }
        }, this.active_tab_speed);

	//
	// announce we are alive to trigger visibility check in other tabs
	//
        channel.postMessage({ connected : 1 });

	//
	// and force options reload when the active tab saves it....
	//
        window.addEventListener('storage', (e) => {
          if (this.active_tab == 0) {
console.log("----------------------------------------");
console.log("-- REFRESHING OPTIONS on INACTIVE TAB --");
console.log("----------------------------------------");
          this.loadOptions();
        };
      });
