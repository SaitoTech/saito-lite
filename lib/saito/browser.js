  ...
  async initialize(app) {
    ...
    try {
      ...
      try {
        ...
        if (!document.hidden) {
          channel.postMessage({ active : 1 , publickey : app.wallet.returnPublicKey() });
	}
	    ...
	    if (e.data) {
	      if (e.data.active == 1) {
	        if (e.data.active == 1 && e.data.publickey === app.wallet.returnPublicKey()) {
                  this.active_tab = 0;
	        }
	      }
	    }
	...
	document.addEventListener("visibilitychange", () => {
  	  if (document.hidden) {
            channel.postMessage({ active : 0 , publickey : app.wallet.returnPublicKey() });
	  } else  {
	    this.active_tab = 1;
            channel.postMessage({ active : 1 , publickey : app.wallet.returnPublicKey() });
	  }
          ...
          for (let i = 0; i < available_cryptos.length; i++) {
            if (available_cryptos[i].ticker) {
              if (available_cryptos[i].ticker.toLowerCase() === pair[1].toLowerCase()) {
                ...
                this.app.wallet.setPreferredCrypto(available_cryptos[i].name);
              }
            }
          }
