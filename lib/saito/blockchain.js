...
console.log(" ... prevhash: " + blk.block.prevbsh);
...
console.log(" ... adding to stakers w/ empty array at pos: " + pos);
    this.index.paths.splice(pos, 0, [])
    this.index.stakers.splice(pos, 0, [])
...
console.log("Setting LXC 2 -- that bsh is LC");
        i_am_the_longest_chain = 1;
...
console.log("Previous block is: " + lblk.returnHash());
...
console.log(blk.block.id + "<---->" + this.index.blocks[this.lc_pos].block.id);
console.log(blk.block.prevbsh + " -- " + this.last_bsh);
...
console.log("LONGEST CHAIN!!! heading into handleTransactionSlips and then validate");
...
console.log("ADDING STAKING SLIP !" + i + " -- " + ii + " at pos " + pos);
console.log("is the slip still here? " + JSON.stringify(blk.transactions[i].transaction.to[ii]));
...
console.log("CWI: " + current_wind_index);
	...
	does_block_validate = await blk.validate();
      ...
      if (new_blk == null) {
        ...
        //
console.log("block hash: " + this_block_hash);
	//
	//
	//
let blks = this.returnBlockByHashFromBlockIndex(this_block_hash);
...
console.log("current_wind_index: " + current_wind_index);
...
console.log("Add Block to Blockchain Success: " + blk.returnHash());
...
console.log("STAKING LENGTH: " + this.index.stakers.length + " ---> " + prevpos);
      }
    ...
    //
      ...
      try {
        for (let i = 0; i < this.index.stakers[pos].length; i++) {
console.log("i is: " + i);
console.log("stakers length is: " + this.index.stakers[pos].length);
 	  this.index.stakers[pos][i].paid = 0;
        }
      } catch (err) {
...
console.log("RWS: start of function");
...
console.log("RWS: before stakers length");
...
console.log("i is " + i + " ---> " + JSON.stringify(this.index.stakers[i]));
...
console.log("selected winning slip: " + winner_pos + "-" + winner_sid);
