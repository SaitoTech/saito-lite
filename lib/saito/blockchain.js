...
console.log("Add Block to Blockchain Success: " + blk.returnHash() + " -- " + force);
...
//    }
  ...
  returnBlockPos(bid, pos=-1) {
    ...
    if (this.index.blocks.length < pos) { 
      return -1; 
    }
    ...
    //
    // roll up until we are at the latest block, or are ABOVE the target BID with the longest chain 
    //
    for (let i = pos; i < this.index.blocks.length-1; i++) {
      if (this.index.blocks[i].block.id >= bid) {
        if (this.bsh_lc_hmap[this.index.blocks[i].returnHash()] == 1) {
          high_tide = i;
	  i = this.index.blocks.length+1;
	}
      }
    }


    if (high_tide == -1) { high_tide = pos; }

    //
    // pos is now highest bid at current
    //
    if (this.index.blocks[high_tide].block.id >= bid) {

      //
      // search down
      //
      for (let i = high_tide; i >= 0; i--) {
        if (this.index.blocks[i].block.id == bid) {
          if (this.bsh_lc_hmap[this.index.blocks[i].returnHash()] == 1) {
	    return i;
  	  }
