...
console.log("2");
  ...
  returnWinningStaker(gt, gtrandom=null, bid, step="validate") {
...
console.log("STAKERS: " + JSON.stringify(this.index.stakers));
	...
	lss_sid = this.last_staker_sid;
	lss_bid = this.last_staker_bid;
	lss_pos = this.last_staker_pos;

        //
        // sanity check
        //
        if (lss_sid < 0 && this.index.stakers[lss_pos].length > 0) {
	  lss_sid = this.index.stakers[lss_pos].length-1;
	}

	//
	// winning slip is previous of this slip
	//
	let last_stkr = this.index.stakers[lss_pos][lss_sid];

	lss_bid = last_stkr.last_bid;
	lss_sid = last_stkr.last_sid;
        lss_pos = this.returnBlockPos(lss_bid, lss_pos);
