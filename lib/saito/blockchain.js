...
***REMOVED***
    ...
    // insert indexes //
    ////////////////////
    let pos = this.binaryInsert(this.index.blocks, blk, (a, b) => { return a.block.ts - b.block.ts; ***REMOVED***);
...
console.log(i_am_the_longest_chain + " ----> LAST BSH: " + this.last_bsh + " --- " + blk.block.prevbsh);
...
console.log("SEARCHING FOR ANCESTOR at position " + search_pos);
...
console.log("common ancestor found at: " + search_pos);
...
console.log("keep looking 2!");
              if (search_bsh == nchain_prevbsh) {
console.log("keep looking 2-1!");
                nchain_prevbsh = this.index.blocks[search_pos].block.prevbsh;
console.log("keep looking 2-2!");
                nchain_len++;
console.log("keep looking 2-3!");
                nchain_bf = nchain_bf + this.index.blocks[search_pos].block.bf;
console.log("keep looking 2-4!");
          ***REMOVED***

console.log("keep looking 3!");
              shared_ancestor_pos = search_pos;
              search_pos--;
      ...
      ***REMOVED***
console.log("keep looking 4!");
              if (shared_ancestor_pos == 1) {
                if (nchain_prevbsh == "") {
console.log("new chain disconnected 1");
                  await this.addBlockToBlockchainSuccess(blk, pos, 0);
                  return;
            ***REMOVED***
          ***REMOVED***
console.log("keep looking 5!");
              if (shared_ancestor_pos == 0) {
                if (nchain_prevbsh != lchain_prevbsh) {
console.log("new chain disconnected 2");
                  await this.addBlockToBlockchainSuccess(blk, pos, 0);
                  return;
            ***REMOVED***
console.log("keep looking 6!");
          ***REMOVED***
console.log("keep looking 7!");
        ***REMOVED***

console.log("keep looking 8: " + search_pos);
...
console.log("new chain longest 3");
            i_am_the_longest_chain = 1;
      ***REMOVED***
    ***REMOVED***
  ***REMOVED*** else {
...
console.log("INSERT POSITION OF THIS BLOCK: " + pos);
...
console.log("WE JUST SET LAST_BSH TO: " + this.last_bsh);

***REMOVED***

console.log(i_am_the_longest_chain + " <--- is it the longest chain?");
...
***REMOVED***
    ...
    if (i_am_the_longest_chain == 1 && this.index.blocks.length > 0) {
      ...
      new_block_hashes     = [];
      old_block_hashes     = [];
      if (this.last_lc_pos != pos) { old_hash_to_hunt_for = this.index.blocks[this.last_lc_pos].returnHash(); ***REMOVED***
...
console.log("DONE LOADING: " + this.last_bid + " -- " + this.last_bsh);
...
***REMOVED***
***REMOVED*** the block does not validate, unwind
***REMOVED***
        if (current_wind_index == 0) {

  ***REMOVED*** this is the first block we have tried to add
  ***REMOVED*** and so we can just roll out the older chain
  ***REMOVED*** again as it is known good.
  ***REMOVED***
  ***REMOVED*** note that old and new hashes are swapped
  ***REMOVED*** and the old chain is set as null because
  ***REMOVED*** we won't move back to it. we also set the
  ***REMOVED*** resetting_flag to 1 so we know to fork
  ***REMOVED*** into addBlockToBlockchainFailure
  ***REMOVED***
          if (old_block_hashes.length > 0) {
            this.windChain(
              blk,
...
***REMOVED***
  ***REMOVED***
  ...
  returnLastSharedBlockId(fork_id, latest_known_block_id) {
    ...
    if (fork_id == null || fork_id == "") { return 0; ***REMOVED***
    if (fork_id.length < 2) { if (latest_known_block_id > 0) { latest_known_block_id - 1; ***REMOVED*** else { return 0; ***REMOVED*** ***REMOVED***
...
***REMOVED***
    ...
    for (let fii = 0; fii < (fork_id.length/2); fii++) {
      ...
      if (fii == 0)  { our_fork_id_pair_blockid = latest_known_block_id - 0; ***REMOVED***
      if (fii == 1)  { our_fork_id_pair_blockid = latest_known_block_id - 10; ***REMOVED***
      if (fii == 2)  { our_fork_id_pair_blockid = latest_known_block_id - 20; ***REMOVED***
      if (fii == 3)  { our_fork_id_pair_blockid = latest_known_block_id - 30; ***REMOVED***
      if (fii == 4)  { our_fork_id_pair_blockid = latest_known_block_id - 40; ***REMOVED***
      if (fii == 5)  { our_fork_id_pair_blockid = latest_known_block_id - 50; ***REMOVED***
      if (fii == 6)  { our_fork_id_pair_blockid = latest_known_block_id - 75; ***REMOVED***
      if (fii == 7)  { our_fork_id_pair_blockid = latest_known_block_id - 100; ***REMOVED***
      if (fii == 8)  { our_fork_id_pair_blockid = latest_known_block_id - 200; ***REMOVED***
      if (fii == 9)  { our_fork_id_pair_blockid = latest_known_block_id - 500; ***REMOVED***
      if (fii == 10) { our_fork_id_pair_blockid = latest_known_block_id - 1000; ***REMOVED***
      if (fii == 11) { our_fork_id_pair_blockid = latest_known_block_id - 5000; ***REMOVED***
      if (fii == 12) { our_fork_id_pair_blockid = latest_known_block_id - 10000; ***REMOVED***
      if (fii == 13) { our_fork_id_pair_blockid = latest_known_block_id - 50000; ***REMOVED***
...
***REMOVED*** if we have not found a match, return 0 since we have
...
***REMOVED*** become the longest chain
        if (tmpklr == "") { return 0; ***REMOVED***

        var our_fork_id_pair = tmpklr.substring(0, 2);

***REMOVED*** if we have a match in fork ID at a position, treat this
...
***REMOVED***
  ...
  ***REMOVED***
  ...
  returnLongestChainIndexPosArray(chainlength=10) {
    ...
    if (this.index.blocks.length == 0) { return []; ***REMOVED***
    if (this.index.blocks.length < chainlength) { chainlength = this.index.blocks.length; ***REMOVED***
    if (chainlength == 0) { return []; ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
  ...
  updateForkId(blk) {
    ...
    if (blk == null) { return this.fork_id; ***REMOVED***
...
***REMOVED***
    ...
    for (let i = 0, stop = 0; stop == 0 && i < this.genesis_period;) {
      ...
      if (indexpos == -1 || checkpointblkid < 0) { stop = 1; ***REMOVED***
      else {
  ...
  ***REMOVED***
      ...
      //
      if (i == 10000) { i = 50000; ***REMOVED***
      if (i == 5000)  { i = 10000; ***REMOVED***
      if (i == 1000)  { i = 5000; ***REMOVED***
      if (i == 500)   { i = 1000; ***REMOVED***
      if (i == 200)   { i = 500; ***REMOVED***
      if (i == 100)   { i = 200; ***REMOVED***
      if (i == 75)    { i = 100; ***REMOVED***
      if (i == 50)    { i = 75; ***REMOVED***
      if (i == 40)    { i = 50; ***REMOVED***
      if (i == 30)    { i = 40; ***REMOVED***
      if (i == 20)    { i = 30; ***REMOVED***
      if (i == 10)    { i = 20; ***REMOVED***
      if (i == 0)     { i = 10; ***REMOVED***

      if (i > this.genesis_period || i == 50000) { stop = 1; ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
  ...
  isHashIndexed(hash) {
    if (this.bsh_bid_hmap[hash] > 0) { return true; ***REMOVED***
    return false;
  ***REMOVED***

  returnLatestBlock() {
    if (this.index.blocks.length == 0) { return null; ***REMOVED***
...
***REMOVED***
  ***REMOVED***
  ...
  async returnBlockById(bid, mode=0) {
    ...
    if (hash == undefined) { return null; ***REMOVED***
...
***REMOVED***
  ***REMOVED***
  ...
  returnMinTxId() {
    if (this.lc_pos == null) { return 0; ***REMOVED***
    if (this.index.blocks[this.lc_pos].mintxid == undefined) { return 0; ***REMOVED***
    return this.index.blocks[this.lc_pos].maxtxid;
  ***REMOVED***

  returnMaxTxId() {
    if (this.lc_pos == null) { return 0; ***REMOVED***
    if (this.index.blocks[this.lc_pos].maxtxid == undefined) { return 0; ***REMOVED***
    return this.index.blocks[this.lc_pos].maxtxid;
  ***REMOVED***
...
console.log("setting last bsh to : " + blk.returnHash());
    this.last_bsh = blk.returnHash();
    this.last_bid = blk.block.id;
    this.last_ts  = blk.block.ts;
    this.last_bf  = blk.block.bf;
