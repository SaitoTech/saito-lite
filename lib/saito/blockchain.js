...
console.log(" trying to find the ancestor...");
...
console.log("lchainbf start: " + lblk.block.bf);
console.log("nchainbf start: " + nblk.block.bf);
...
console.log(" searching at position: " + search_pos);
...
console.log(" found the common ancestor at position: " + search_pos);
              shared_ancestor_pos_found = true;
              shared_ancestor_pos = search_pos;
              search_pos = -1;
console.log("OCH: " + old_chain_bsh);
console.log("NCH: " + new_chain_bsh);
            //
            // keep looking
            //
            } else {
console.log(" did not find the common ancestor at position: " + search_pos);
...
console.log("lchain bf adds: " + this.index.blocks[search_pos].block.bf);
...
console.log("nchain bf adds: " + this.index.blocks[search_pos].block.bf);
		...
		new_chain_pos.push(search_pos);
          ...
          //
console.log("COMPARE: ");
console.log("LEN: " + nchain_len + " -- " + lchain_len);
console.log("BF: " + nchain_bf + " -- " + lchain_bf);
console.log("SAPF: " + shared_ancestor_pos_found);
          if (nchain_len > lchain_len && nchain_bf >= lchain_bf && shared_ancestor_pos_found == true) {
console.log("Setting LXC 5 - " + nchain_len + " -- " + lchain_len);
            i_am_the_longest_chain = 1;
          }
        }
      } else {

console.log("this block id is less than we expected...");
        ...
        if (blk.block.prevbsh == this.last_bsh && blk.block.prevbsh != "") {
          ...
          for (let h = pos+1; h < this.index.blocks.length; h++) {
            ...
            //
console.log("reorganizing all of the block earlier at pos " + h);
            this.onChainReorganization(this.index.blocks[h].block.id, this.index.blocks[h].returnHash(), 0, pos);
          }

console.log("Setting LXC 6 at pos " + pos);
          i_am_the_longest_chain = 1;
...
console.log("lc? " + i_am_the_longest_chain);
...
console.log("VALIDATION HAPPENING HERE 1");
      //
    ...
    //
    if (i_am_the_longest_chain == 0) {
console.log("VALIDATION HAPPENING HERE 2");
      await this.addBlockToBlockchainSuccess(blk, pos, 0);
      await this.handleTransactionSlips(blk, pos);
      this.app.mempool.removeBlockAndTransactions(blk);
      this.indexing_active = 0;
    ...
    //
console.log("VALIDATION HAPPENING HERE 3");
...
console.log("start of validation function");
...
console.log("WIND CHAIN LC: " + i_am_the_longest_chain);
...
console.log("UNWINDING THESE HASHES: " + old_block_hashes);
...
console.log("unwinding chain 1");
  ...
  async addBlockToBlockchainSuccess(blk, pos, i_am_the_longest_chain, force=false) {
    ...
    //
console.log("winding chain in addBlockToBlockchain Sucess: " + i_am_the_longest_chain);
...
//    }
  ...
  async addBlockToBlockchainFailure(blk, pos, i_am_the_longest_chain, force=no) {
    ...
    //
    // indexing_active should be set in parent resume normal operations
    //
    //this.indexing_active = 0;
...
if (lc == 0) {
  console.log("UNSPENDING SLIPS in block " + bid + " ---> " + bsh); 
}
  ...
  async returnBlockByHashFromBlockIndex(hash) {
...
//console.log("ERROR 582034: error trying to update slip at pos: " + pos);
      }
