...
class Blockchain {
  ...
  constructor(app) {
    ...
    this.earliest_unpaid_bid    = -1;
    this.earliest_unpaid_sid    = -1;
    this.latest_unpaid_bid      = -1;
    this.latest_unpaid_sid      = -1;
...
/***
    if (lc == 1) {
      for (let z = 0; z < this.index.stakers[pos].length; z++) {
	if (this.index.stakers[pos].length > 0) {
	  if (this.index.stakers[pos][z].paid == this.staking_round) {
...
console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^");
  ...
  //
  addStakingSlip(blk, slip, pos, round_eligible_for_payment=0) {

    
//
// find next and last on insert checking with block id to keep from infinitely long searches
//
    let x               = {};
        x.slip          = slip;
        x.paid          = -1;     // last staking round in which paid

        x.next_bid      = 0;     // if paid, points to bid of next unpaid
        x.next_sid      = 0;     // if paid, points to sid of next unpaid
        x.last_bid      = 0;     // if paid, points to bid of last unpaid
        x.last_sid      = 0;     // if paid, points to sid of last unpaid

    let bsh = blk.returnHash();

    if (round_eligible_for_payment > 0) {
      x.paid = round_eligible_for_payment;
    }
    ...
    if (this.index.stakers[pos].length > 0) {
      ...
      x.last_bid = blk.block.id;
      x.last_sid = this.index.stakers[pos].length-1;
      x.next_bid = this.index.stakers[pos][this.index.stakers[pos].length-1].next_bid;
      x.next_sid = this.index.stakers[pos][this.index.stakers[pos].length-1].next_sid;

      //
      // if previous is spent, update our last_bid / last_sid to last unpaid
      //
      if (eligible_for_immediate_payment == 1) {
        if (this.index.stakers[pos][x.last_sid].paid > x.paid) {
	  x.last_bid = this.index.stakers[pos][x.last_sid].last_bid;
	  x.last_sid = this.index.stakers[pos][x.last_sid].last_sid;
        }
      } else {
	x.last_bid = this.index.stakers[pos][x.last_sid].last_bid;
	x.last_sid = this.index.stakers[pos][x.last_sid].last_sid;
      ...
      this.index.stakers[pos][this.index.stakers[pos].length-1].next_bid = blk.block.id;
      this.index.stakers[pos][this.index.stakers[pos].length-1].next_sid = this.index.stakers[pos].length;

    } else {

      //
      // TODO - what if previous block is missing
      //
      if (blk.block.prevbsh != "") {
	for (let i = pos-1, current_bid = blk.block.id, target_bid = current_bid-1; i > 0 && this.index.blocks[i].block.id >= target_bid; i--) {
	  if (this.index.blocks[i].returnHash() == blk.block.prevhsh) {

            x.last_bid = blk.block.id;
            x.last_sid = this.index.stakers[i].length-1;
            x.next_bid = this.index.stakers[i][this.index.stakers[i].length-1].next_bid;
            x.next_sid = this.index.stakers[i][this.index.stakers[i].length-1].next_sid;

            //
            // if previous is spent, update our last_bid / last_sid to point to ITS previous
            //
            if (eligible_for_immediate_payment == 1) {
              if (this.index.stakers[pos][x.last_sid].paid > x.paid) {
  	        x.last_bid = this.index.stakers[i][x.last_sid].last_bid;
	        x.last_sid = this.index.stakers[i][x.last_sid].last_sid;
              }
            } else {
  	      x.last_bid = this.index.stakers[i][x.last_sid].last_bid;
	      x.last_sid = this.index.stakers[i][x.last_sid].last_sid;
            ...
            this.index.stakers[i][this.index.stakers[i].length-1].next_bid = blk.block.id;
            this.index.stakers[i][this.index.stakers[i].length-1].next_sid = this.index.stakers[i].length;
    ...
    //
    // loop forward to check if we need to update next slip referencing up
    //
    for (let i = pos+1, current_bid = blk.block.id, target_bid = current_bid+1; i < this.index.blocks.length && this.index.blocks[i].block.id <= target_bid; i++) {
      if (this.index.blocks[i].block.prevbsh == bsh) {

//
// update the next block and this block to hook them up
//
    ...
    console.log("LINKED-LIST: " + JSON.stringify(x));
  ...
  updateStakingRound(max_bid) {
    ...
    this.staking_round++;
    this.staking_round_min_bid[this.staking_round] = this.staking_round_max_bid[this.staking_round-1]+1;
    this.staking_round_max_bid[this.staking_round] = max_bid;

    //
    // start measuring the next pool
    //
    //this.staking_pool[(this.staking_round+1)] = "0.0";
    ...
    console.log("num current stakers: " + this.stakers_current);
    console.log("num pending stakers: " + this.stakers_pending);
  ...
  returnWinningStaker(gt, gtrandom=null, bid) {
    ...
    for (let i = this.lc_pos, k = 0; i >= 0 && k == 0; i--) {
      ...
      if (this.index.blocks[i].block.id == min_pos) {
        if (this.bsh_lc_hmap[this.index.blocks[i].returnHash()] == 1) {
	  min_pos = i;
	  k = 1;
        }
    ...
    if (this.index.stakers[winner_pos].length == 0) {
      for (let i = winner_pos; i > 0 && this.index.blocks[i].block.id >= min_bid;) {
        if (this.bsh_lc_hmap[this.index.blocks[i].returnHash()] == 1) {
          if (this.index.stakers[i].length == 0 && this.index.blocks[i-1].block.id >= min_bid) {
	    winner_pos = i-1;
          } else {
	    i = -1;
	  }
    ...
    console.log("SEARCHING MIN BID: " + this.staking_round_min_bid[this.staking_round]);
    console.log("SEARCHING MAX BID: " + this.staking_round_max_bid[this.staking_round]);
    console.log("num current stakers: " + this.stakers_current);
    ...
    let winning_slip_num2 = (winnerNum2 % this.index.stakers[winner_pos].length);

    let winner_slip = this.index.stakers[winner_pos][winning_slip_num2];

    let starting_pos  = winner_pos;
    let starting_bid  = winner_bid;
    let starting_sid  = winner_sid;
    let starting_slip = winner_slip;

console.log("SLIPS: " + JSON.stringify(this.index.stakers));
console.log("WINNER: " + JSON.stringify(winner_slip));
console.log(winner_pos + " -- " + winner_bid + " -- " + winner_sid);
...
console.log("...");
      ...
      //
      for (let i = winner_pos; i > 0 && i >= min_bid; i--) {

	//
	// we find our block
	//
	if (this.index.blocks[i].block.id == next_bid && this.bsh_lc_hmap[this.index.blocks[i].returnHash()] == 1) {

	  winner_bid = this.index.blocks[i].block.id;
	  winner_sid = next_sid;
	  winner_pos = i;
	  winner_slip = this.index.stakers[i][next_sid];

	  starting_slip.next_bid = this.index.stakers[i].next_bid;
	  starting_slip.next_sid = this.index.stakers[i].next_sid;
	...
	if (winner_slip.next_bid == 0 && winner_slip.next_sid == 0 && winner_slip.pending == 0) {
	  ...
	  next_bid = max_bid;
          winner_pos = max_pos;
	  next_sid = this.index.stakers[winner_pos].length-1;
          winner_slip = this.index.stakers[winner_pos][next_sid];
          winner_sid = next_sid;
    ...
    if (winner_slip.pending == 1) {
      winner_slip.pending = 0;
      this.stakers_current--;
    }
