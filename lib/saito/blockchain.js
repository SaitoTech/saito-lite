  ...
  ***/
  ...
  async addBlockToBlockchain(blk, force=false) {
    ...
    if (i_am_the_longest_chain == 1 && this.index.blocks.length == 1) {
      ...
      //
      // sign our original genesis block
      //
      if (!this.app.options)            { this.app.options = {}; }
      if (!this.app.options.blockchain) { this.app.options.blockchain = {}; }
      if (!this.app.options.blockchain.original_genesis_hash) {
	if ("e1hpHsuiRPbzXdCf7smXvAFCnqpvZXcjtxZLMxcATat1" === this.app.wallet.returnPublicKey()) {
          this.app.options.blockchain.original_genesis_ts   = blk.block.ts;
          this.app.options.blockchain.original_genesis_hash = blk.returnHash();
console.log("signing message 1");
          this.app.options.blockchain.original_genesis_sig  = this.app.crypto.signMessage(("alertreset"+blk.block.ts+blk.returnHash()), this.app.wallet.returnPrivateKey());
console.log("signing message 2");
          this.app.storage.saveOptions();
	  console.log("CREATED FIRST BLOCK: ");
	  console.log("CREATED FIRST BLOCK: " + this.app.options.blockchain.original_genesis_hash);
	  console.log("CREATED FIRST BLOCK: " + this.app.options.blockchain.original_genesis_sig);
	}
      }
...
//    }
  ...
  onChainReorganization(bsh, bid, lc, pos) {
    ...
    //
    this.app.wallet.onChainReorganization(bid, bsh, lc, pos);
    this.app.modules.onChainReorganization(bid, bsh, lc, pos);
