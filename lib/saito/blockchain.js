...
console.log("ABTB sTART: " + JSON.stringify(this.index.stakers));
...
console.log("current_unwind_index is: " + current_unwind_index);
//console.log("and old_chain_pos: " + JSON.stringify(old_chain_pos));
...
console.log("STKER PRE UPDATE: " + JSON.stringify(this.index.stakers));
    this.updateStakingSlip(pos, prevpos, lc);
console.log("STKER POST UPDATE: " + JSON.stringify(this.index.stakers));
...
console.log(JSON.stringify(this.index.stakers));
  ...
  updateStakingSlip(pos, prevpos, lc) {
    ...
    //
    // set as unpaid, update next / last
    //
    if (lc == 1) {
	...
	slip.paid = 0;
        ...
        //
        // if this is the first slip in our new block
        //
        if (i == 0) {

console.log("\n\n\n\ntry to find previous slip pos and sid");

	  //
	  // find previous block
	  //
          let prevbsh = this.index.blocks[pos].block.prevbsh;
          for (let ii = pos-1, cont = 1; cont == 1 && ii >= 0; ii--) {
console.log("looking for " + prevbsh);
console.log("trying pos: " + ii);
console.log("bsh at this position is: " + this.index.blocks[ii].returnHash());

            if (prevbsh == this.index.blocks[ii].returnHash()) {
	      prevpos = ii;
console.log("found " + prevbsh);

	      if (this.index.stakers[prevpos].length > 0) {
console.log("we have stakers at pos: " + prevpos);
		cont = 0;
	      } else {
console.log("no stakers at pos: " + prevpos);
	        prevbsh = this.index.blocks[prevpos].block.prevbsh; 
	      }
	    }
          }
console.log("PREVPOS is: " + prevpos);
	  ...
	  //
	  // or set as our last block
	  //
	  } else {

	    slip.last_sid = this.index.stakers[prevpos].length-1;
...
console.log("POS is: " + prevpos);

	  //
	  // last in this block is last
	  //
	  slip.last_sid = i-1;
	  slip.last_bid = this.index.blocks[pos].block.id;
	  slip.next_bid = -1;
  	  slip.next_sid = -1;
        ...
        //
        // update previous slip if exists
        //
        if (slip.last_bid != -1) {

console.log("PREV SLIP EXISTS: ");
console.log(slip.last_bid + " -- " + slip.last_sid);

          prevpos = this.returnBlockPos(slip.last_bid, slip.last_sid);

console.log(prevpos);

          this.index.stakers[prevpos][slip.last_sid].next_bid = this.index.blocks[pos].block.id;
console.log("1");
          this.index.stakers[prevpos][slip.last_sid].next_sid = i;
console.log("2");
    ...
    //
      ...
      for (let i = 0; i < this.index.stakers[pos].length; i++) {
        ...
        if (i == 0) {
          ...
          prevpos = this.returnBlockPos(slip.last_bid, slip.last_sid);
	  let ppsid = this.index.stakers[prevpos].length-1;

          this.index.stakers[prevpos][ppsid].next_bid = -1;
          this.index.stakers[prevpos][ppsid].next_sid = -1;
  ...
  returnWinningStaker(gt, gtrandom=null, bid) {
    ...
    for (let i = winner_pos, k = 0; i >= 0 && k == 0; i--) {
      //
      // extra check with bid is sanity as bundle won't consider the new block longest-chain yet
      //
      if (this.bsh_lc_hmap[this.index.blocks[i].returnHash()] == 1 || this.index.blocks[i].block.id == bid) {
        if (k == winner_bid) {
	  winner_pos = i;
	  k = 1;
        }
...
console.log("winnerNum2 + ------> " + winnerNum2);
console.log("winnerPos2 + ------> " + winner_pos);
console.log("winner_sid + ------> " + winner_sid);
console.log("stakerslen + ------> " + this.index.stakers[winner_pos].length);
...
console.log("STAKERS: " + JSON.stringify(this.index.stakers));
    ...
    //
      ...
      //
      this.last_staker_pos = this.returnBlockPos(this.last_staker_bid, this.last_staker_pos);


      //
      // last spend slip exists...
      //
      if (this.last_staker_pos != -1) {

        //
        // sanity check
        //
        if (this.last_staker_sid < 0 && this.index.stakers[this.last_staker_pos].length > 0) {
	  this.last_staker_sid = this.index.stakers[this.last_staker_pos].length-1;
	}

	//
	// winning slip is previous of this slip
	//
	let last_stkr = this.index.stakers[this.last_staker_pos][this.last_staker_sid];

	this.last_staker_bid = last_stkr.last_bid;
	this.last_staker_sid = last_stkr.last_sid;
        this.last_staker_pos = this.returnBlockPos(this.last_staker_bid, this.last_staker_pos);

	//
	// first one
	//
        if (this.last_staker_bid == -1 && this.last_staker_sid == -1) {
	  if (this.last_staker_pos == 0) {
	    this.last_staker_sid = 0;
	    this.last_staker_bid = this.index.blocks[this.last_staker_pos].block.id;
	  }
	}
	...
	console.log("or it has slipped off the chain... in whch case we go to the end and start there");

	for (let i = this.index.stakers.length-1; i >= 0; i--) {
	  for (let ii = this.index.stakers[i].length-1; ii >= 0; ii--) {
	    this.last_staker_pos = i;
	    this.last_staker_sid = ii;
	    this.last_staker_bid = this.index.blocks[i].block.id;;
      ...
      //
      // winning slip is latest
      //
      winner_bid = this.last_staker_bid;
      winner_sid = this.last_staker_sid;
      winner_pos = this.last_staker_pos;
      winner_slip = this.index.stakers[winner_pos][winner_sid];
