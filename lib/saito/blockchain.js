...
console.log("adding hash to blockchain!");
...
console.log("shared ancestor pos = " + shared_ancestor_pos);
        ...
        if (blk.block.prevbsh == this.last_bsh && blk.block.prevbsh != "") {
          ...
          //
console.log("on chain reorg 4 - with: " + pos);
...
console.log("am i the longest chain? " + i_am_the_longest_chain);
    ...
    if (i_am_the_longest_chain == 1 && this.index.blocks.length == 1) {
      ...
      //
console.log("validation process 1");
    ...
    //
    if (i_am_the_longest_chain == 0) {
console.log("validation process 2");
      await this.handleTransactionSlips(blk, pos);
      await this.addBlockToBlockchainSuccess(blk, pos, 0);
      this.indexing_active = 0;
      return;
...
console.log("validation process 3");
    await this.handleTransactionSlips(blk, pos);
    await this.validate(
      blk,
      pos,
...
console.log("checking if block validates!");


//
// HACK - we are returning instead of continuing
//
      let does_block_validate = 0;

      if (new_blk.validates == 1) { 
	does_block_validates = 1;
      } else {
	does_block_validate = new_blk.validate();
      }

      if (does_block_validate) {

console.log("on chain reorg 5 - with: " + new_chain_pos[current_wind_index]);
...
console.log("moving into wind index...: ");
...
console.log("on chain reorg 1 - with: " + old_chain_pos[current_unwind_index]);
...
console.log("Add Block to Blockchain Success: " + blk.returnHash());
    ...
    //
console.log("on chain reorg 2 - with: " + pos);
...
console.log("on chain reorg 3 - with: " + pos);
  ...
  returnBlockPos(bid, pos=-1) {
    ...
    if (pos == -1) { 
      ...
      for (let i = pos; i >= 0 && this.index.blocks[i].block.id >= bid; i--) {
        if (this.index.blocks[i].block.id == bid) {
    ...
    if (this.index.blocks[pos].block.id >= bid) {
      ...
      for (let i = pos; i < this.index.blocks.length-1 && this.index.blocks[i].block.id <= bid; i++) {
        if (this.index.blocks[i].block.bid == bid) {
          if (this.bsh_lc_hmap[this.index.blocks[i].returnHash()] == 1) {
	    return i;
	  }
	}
...
console.log("return block by hash from memory or disk 1");
    let blk = await this.returnBlockByHash(hash, mode);
console.log("return block by hash from memory or disk 2");
    if (blk == null) {
      blk = this.returnBlockByHashFromDisk(hash);
console.log("return block by hash from memory or disk 3");
      return blk;
    } else {
console.log("return block by hash from memory or disk 4");
      return blk;
    }
console.log("return block by hash from memory or disk 5");
    return null;
  }
...
console.log("ERROR 582035: error trying to unspend slip at pos: " + pos);
  ...
  returnWinningStaker(gt, gtrandom=null, bid) {
    ...
    //
      ...
      let p = this.returnBlockPos(earliest_bid, this.lowest_unspent_pos);
      let cont = 1;

      //
      // TODO randomized order not just first-always wins
      //
      for (let i = p; i < this.index.stakers.length && cont == 1; i++) {
