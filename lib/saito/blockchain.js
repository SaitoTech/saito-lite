...
class Blockchain {
  ...
  constructor(app) {
    ...
    this.index.paths		= [];
    this.index.stakers          = [];
    this.index.staking_reward   = [];
    this.index.cumulative_stake = [];
...
console.log("\n\n ... adding block "+blk.block.id+": " + blk.returnHash());
    ...
    this.index.staking_reward[pos] = [];
    this.index.cumulative_stake[pos] = "0.0"; // string of amount staked
...
console.log("^^^^^^^^^^^^^^^^^^^^^^^^^^");
    ...
    this.index.cumulative_stake.splice(0, items_before_needed);
  ...
  addStakingSlip(blk, slip, pos) {
    ...
    this.index.cumulative_stake[pos] = Big(this.index.cumulative_stake[pos]).plus(Big(slip.amt)).toFixed(8);
  ...
  returnWinningStaker(gt, gtrandom=null) {

    if (gtrandom == null) { gtrandom = gt.random; }

console.log("picking staker for round: " + this.staking_round);

    //
    // pick winning staker
    //
    if (this.stakers_current == 0) { this.updateStakingRound(this.last_bid); }
    ...
    //
console.log("pre min / max for loop");
...
console.log("post min / max for loop");
...
console.log("pre cycle down");
    ...
    if (this.index.stakers[winner_pos].length == 0) {
      for (let i = winner_pos; i > 0 && this.index.blocks[i].block.id >= min_bid;) {
        if (this.bsh_lc_hmap[this.index.blocks[i].returnHash()] == 1) {
          if (this.index.stakers[i].length == 0) {
	    winner_pos = i-1;
          } else {
	    i = -1;
	  }
...
console.log(JSON.stringify(this.index.stakers));
    ...
    let winner_slip = this.index.stakers[winner_pos][(winnerNum2 % this.index.stakers[winner_pos].length)];
