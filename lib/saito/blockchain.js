...
console.log(JSON.stringify(this.app.blockchain.index.staking));
  ...
  returnWinningStaker(gt, stakers=[], gtrandom) {
    ...
    if (this.stakers_current == 0) { return results; }
...
console.log("starting winner bid is " + winner_bid);
console.log("min pos: " + min_pos);
console.log("min bid: " + min_bid);

    for (let i = min_pos, k = min_bid; k <= winner_bid;) {
console.log("is block at pos " + i + " on the longest chain? " +  this.bsh_lc_hmap[this.index.blocks[i].returnHash()]);
      if (this.bsh_lc_hmap[this.index.blocks[i].returnHash()] == 1) {
        if (k == winner_bid) {
	  winner_pos = i;
        }
...
console.log("STARTER SID: " + winner_sid + " -- " + starting_sid);
...
console.log("winner_slip: " + JSON.stringify(winner_slip));
...
console.log("\n\nStaking Winner: " + winner_pos + " --- " + winner_bid + " -- " + winner_sid);
console.log(JSON.stringify(winner_slip));
    ...
    results.publickey = winner_slip.add;
