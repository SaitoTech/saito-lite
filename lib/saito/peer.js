...
class Peer {
  ...
  returnHandshake() {
	...
	if (this.app.options.server) {
	  ...
	  //
	  // running on localhost and don't have known host / port / protocol
	  //
...
*/
  ...
  connect(mode = 0) {
    ...
    if (mode == 0) {
      ...
      this.socket = io(`${this.peer.protocol}://${this.peer.host}:${this.peer.port}`, opts);
    ...
    //
    if (mode == 1) { this.addSocketEvents(); }
  ...
  sendRequestWithCallback2(request, data = {}, callback) {
    var requestMessage = {};
...
//console.log("TRIGGERING DISCONNECT ---> WAITING");
//
//
//        this.app.connection.emit('peer_disconnect', this);
      ...
      this.socket.on('request', async (data, mycallback = null) => {
        ...
        if (this.app.BROWSER == 1) {
          if (this.handle_peer_requests == 0) { 

            if (message.request != "keylist" && message.request != "handshake") {
	      console.log("---------------------------------------------");
...
console.log("received: " + JSON.stringify(message.request));

        switch (message.request) {
	  //
	  // TODO - remove alertreset for production
	  //
          case 'alertreset':
            //this.socket.disconnect(true);
            //this.app.network.cleanupDisconnectedSocket(this);
            //this.handleAlertResetRequest(message);
            break;
          case 'handshake':
  ...
  handleAlertResetRequest(message) {

    let original_genesis_hash = "";
    let original_genesis_sig = "";
    let original_genesis_ts = "";

    if (message) {
      if (message.data) {
        if (message.data.original_genesis_hash) { original_genesis_hash = message.data.original_genesis_hash; }
        if (message.data.original_genesis_sig) { original_genesis_sig = message.data.original_genesis_sig; }
        if (message.data.original_genesis_ts) { original_genesis_ts = message.data.original_genesis_ts; }
      }
    }
...
/**** MAY 29 BUG
    if (this.app.options) {
      if (this.app.options.blockchain) {
        if (original_genesis_hash != this.app.options.blockchain.original_genesis_hash) {

          if (this.app.options.blockchain.original_genesis_ts) {
            if (this.app.options.blockchain.original_genesis_ts > original_genesis_ts) {
	      return;
            }
          }

          let msg_to_validate = "alertreset" + original_genesis_ts + original_genesis_hash;
          if (this.app.crypto.verifyMessage(msg_to_validate, original_genesis_sig, "e1hpHsuiRPbzXdCf7smXvAFCnqpvZXcjtxZLMxcATat1")) {
            this.app.options.blockchain.original_genesis_ts = original_genesis_ts;
            this.app.options.blockchain.original_genesis_hash = original_genesis_hash;
            this.app.options.blockchain.original_genesis_sig = original_genesis_sig;
...
//console.log("PEERSHAKE: " + JSON.stringify(this.peer));
  ...
  sendAlertReset() {
    ...
    //
    // notify in case of chain reset
    //
    // TODO - obviously remove for production
    //
    if (this.app.options) {
    if (this.app.options.blockchain) {
    if (this.app.options.blockchain.original_genesis_sig != "") {
      let original_chain_message = {};
      original_chain_message.request = "alertreset";
      original_chain_message.data = {};
      original_chain_message.data.original_genesis_ts = this.app.options.blockchain.original_genesis_ts;
      original_chain_message.data.original_genesis_hash = this.app.options.blockchain.original_genesis_hash;
      original_chain_message.data.original_genesis_sig = this.app.options.blockchain.original_genesis_sig;
      this.socket.emit('request', JSON.stringify(original_chain_message));
    }
    }
    }
  ...
  sendBlockchain(start_bid, synctype) {
    ...
    this.socket.emit('request', JSON.stringify(message));
      ...
      console.log("ERROR 471283: sending blockchain in SBTB: "+err);
    }
    return;
  ...
  returnBlockURL(block_hash) {
    let { protocol, host, port } = this.peer;

    if (host == "localhost") {
      if (this.peer.endpoint.host != "localhost") {
	protocol = this.peer.endpoint.protocol;
	host = this.peer.endpoint.host;
	port = this.peer.endpoint.port;
      }
    }
