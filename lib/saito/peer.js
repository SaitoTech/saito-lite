...
class Peer {
  ...
  connect(mode=0) {
    ...
    if (mode == 0) {
      ...
      logger.info("Generating outbound connection request...");
    ...
    if (mode == 1) {
      ...
      logger.info("Incoming connection request, handshake getting initialized...");
      this.addSocketEvents();
  ...
  addSocketEvents() {
    ...
    try {
      ...
      this.socket.on('connect', () => {
        ...
        logger.info("socket connection has fired!");
      ...
      this.socket.on('request', async (data, mycallback = null) => {
        ...
        if (data === undefined) { 
          logger.info("ERROR 012482: received undefined data from peer.");
          return;
        }
        ...
        //  }
        //} catch (err) {
        //  logger.info("Error triggered by: " + JSON.stringify(message));
        //  logger.info(err);
        //}
...
*/
          default:
logger.info("sending into handlePeerRequest: " + JSON.stringify(message));
            this.app.modules.handlePeerRequest(message, this, mycallback);
            //if (mycallback != null) {
            //  mycallback();
            //}
      ...
      logger.error("ERROR 581023: error handling peer request - " + err);
    }
  }
  ...
  handleHandshakeRequest(message) {
    ...
    logger.info("\n\n\n\n\nHR:");
    logger.info("handleHandshakeRequest");
    ...
    if (this.handshake.challenge_mine != "" && peershake.challenge_proof != "" && this.handshake.challenge_verified == 0) {
      ...
      if (this.app.crypto.verifyMessage("_" + this.handshake.challenge_mine, peershake.challenge_proof, peershake.peer.publickey) == 0) {
        ...
        this.app.connection.emit("handshake_complete", this.peer);
      }
    }
    ...
    if (my_last_bid > last_shared_bid) {
      ...
      if (peer_last_bid - my_last_bid > this.app.blockchain.genesis_period && peer_last_bid != 0 && this.peer.synctype == "full") {	
        ...
        //
        logger.info("ERROR 184023: peer reports us as fully off-chain with same gp");
        process.exit();
  ...
  handleBlockRequest(message) {
    ...
    logger.info("BLOCK AVAILABLE: " + message.data.bhash);
  ...
  handleBlockchainRequest(message) {
    ...
    for (let i = 0; i < blocks.prehash.length; i++) {
      ...
      if (i == 0) {
        ...
        //
        // if (this.app.BROWSER == 1) {
        //   logger.info("slip sanity check: resetting all slips as invalid >= "+bid);
  ...
  sendBlockchain(start_bid, synctype) {
    ...
    logger.info("Sending the Blockchain from start_bid: " + start_bid);
  ...
  handleTransactionRequest(message, mycallback) {
    ...
    if (tx == null) { logger.info("NULL TX"); return; }
    if (!tx.is_valid) { logger.info("NOT VALID TX"); return; }
