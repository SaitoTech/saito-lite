...
class GameHud {
    ...
    render(app, game_mod) {
        if (game_mod.browser_active == 1) {
            ...
            this.renderMenu(app, game_mod);
            this.renderBody(app, game_mod);
        }
    }

    renderMenu(app, game_mod) {
        Array.from(document.getElementsByClassName('hud-menu')).forEach(hud_menu_el => {
          hud_menu_el.append(elParser(GameHudMenuTemplate(this.menu_function_map)));
        });
    }

    renderBody(app, game_mod) {
      this.renderStatus(app, game_mod);
      this.renderLogs(app, game_mod);
    }
    ...
    renderLogsToHudMenuOverlay() {
        ...
        if (hud_menu) {
            ...
            hud_menu.innerHTML = this.logs.slice(0, this.logs_length)
                                    .map(log => `> <span> ${log} </span>`)
                                    .join('<br>');
	    if (hud_menu.innerHTML === "") {
	      hud_menu.innerHTML = "There are no entries in the log, or it has yet to load.";
	    }
        }
    }
	...
	// fullscreen
        let hud_fullscreen_button = document.getElementById('hud-toggle-fullscreen')
