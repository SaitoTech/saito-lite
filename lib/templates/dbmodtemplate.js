...
console.log(JSON.stringify(values));
	...
	let rows = await this.app.storage.queryDatabase(sql, param, dbname);
	let deleted = 0;
	...
	if (rows) {
	  if (rows.length > 0) {
	    if (typeof rows[0].deleted != 'null') {
	      ...
	      await this.app.storage.executeDatabase(sql, param, dbname);
	      deleted = 1;
    ...
    if (txmsg.request == "Delete Item") {

      let values = txmsg.fields;
      let dbname = values[0].dbname;
      let dbtable = values[0].table;
      let id = values[0].item_id;
      let admin = tx.transaction.from[0].add;
      
      let sql = `UPDATE ${dbtable} SET deleted` + " = 1 WHERE id = $id AND (admin = $admin OR $admin = $super)";
      let params = {
        $value: value,
        $id: item_id,
        $admin: tx.transaction.from[0].add,
        ...
        res.rows.forEach(row => {
          ...
          switch (row.name) {
            ...
            case 'publickey':
              ...
              break;
            default:
              body += "<div>" + row.name.replace(/_/g, " ") + "</div>";
...
console.log("DONE UPDATING: ");
  ...
  // Suggest new function that turns text input into select
  // options can be passed from reference table or hard list
  // and are objects with 'text' and 'value' properties.
  // convertToSelect(element, options) {}
  ...
  deleteDatabase(values) {
    ...
    newtx.transaction.msg.request = "delete";
    newtx.transaction.msg.fields = values;
    newtx = this.app.wallet.signTransaction(newtx);

console.log("DELETE TX: " + JSON.stringify(newtx));
...
console.log("UPDATE TX: " + JSON.stringify(newtx));
...
console.log("INSERTING TX: " + JSON.stringify(newtx));
