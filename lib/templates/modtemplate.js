...
console.log("INIT MOD 2!");
    ...
    let sqldir = `${__dirname***REMOVED***/../../mods/${this.dirname***REMOVED***/sql`;
    let fs = app.storage.returnFileSystem();
console.log("INIT MOD 3!");
    if (fs != null) {
console.log("INIT MOD 4!");
      if (fs.existsSync(path.normalize(sqldir))) {
...
***REMOVED***
  ...
  ***REMOVED***
  ...
  async handlePeerRequest(app, message, peer, mycallback=null) {
	...
	let sql = "SELECT * FROM "+this.db_tables[i];
	if (message.data.where != "") { sql += " WHERE "+message.data.where; ***REMOVED***
	let params = {***REMOVED***;
...
***REMOVED***
  ...
  ***REMOVED***
  ...
  async sendPeerDatabaseRequest(dbname, tablename, select=[], where="", peer=null, mycallback=null) {
    ...
    message.data.select     = JSON.stringify(select);
