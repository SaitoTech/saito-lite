...
class ModTemplate {
  ...
  async handlePeerRequest(app, message, peer, mycallback = null) {
    ...
    //console.log(message.data.select + " - " + message.data.dbname + " - " + message.data.tablename);
    for (let i = 0; i < this.db_tables.length; i++) {

      let expected_request = this.name.toLowerCase() + " load " + this.db_tables[i];

      //console.log("REQUEST: " + expected_request);
      ...
      if (message.request === expected_request || message.request_type == 'rawSQL') {
        ...
        console.log(sql);
        console.log(params);
        //console.log(message.data.dbname);
