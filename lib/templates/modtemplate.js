...
class ModTemplate {
  ...
  constructor(app, path) {
    ...
    // A module which wishes to be active in all contexts of the frontend(i.e.
    // not only when browser_active == 1) can set this alwaysRun to true.
    this.alwaysRun = false;
    // A module which wants for a subset of it's methods to always be run
    // can leave alwaysRun set to false and populate this list with pointers
    // to any functions which should always run.
    // e.g. this.alwaysRunThese = [this.initialize, this.onConfirmation]
    this.alwaysRunThese = [];
  ...
  browserize(template) {
    template.initialize = template.onlyInBrowser(template.initialize.bind(template));
    template.initializeHTML = template.onlyInBrowser(template.initializeHTML.bind(template));
    template.updateBalance = template.onlyInBrowser(template.updateBalance.bind(template));
    template.render = template.onlyInBrowser(template.render.bind(template));
    template.attachEvents = template.onlyInBrowser(template.attachEvents.bind(template)); // deprecated
  }
  appify(template) {
    template.initialize = template.onlyOnActiveBrowser(template.initialize.bind(template));
    template.initializeHTML = template.onlyOnActiveBrowser(template.initializeHTML.bind(template));
    template.updateBalance = template.onlyOnActiveBrowser(template.updateBalance.bind(template));
    template.render = template.onlyOnActiveBrowser(template.render.bind(template));
    template.attachEvents = template.onlyOnActiveBrowser(template.attachEvents.bind(template)); // deprecated
  }
  ...
  //
  // DEPRECATION WARNING: respondTo will be deprecated in the future, please
  // use requestInterface instead.
  respondTo(request_type = "") { return null; }

  //
  // Modules may call modules.requestInterfaces(request_type) to get an API
  // from any modules who wish to implement the interface for that type of
  // request. For example, modules that wish to be included in the header might
  // return an interface like {getIcon: "something.png", getUrl: () => {...}} so
  // that the header can render an href that points back to the module. The
  // header would then call module.requestInterfaces("header-links") and each
  // module that wishes to be included in the header would override
  // this.requestInterface such that
  requestInterface(request) { return null; }
...
//        console.log("callback data2: " + JSON.stringify(res));
  ...
  //ToDo - flesh out to include
  //bad chars check
  //default to only accepting SELECTS
  isSQLSafe (sql) {
    return true;
  }
