...
console.log("HANDLE PEER REQUEST: " + message.request + " -- " + message.data.sql);

    if (message.data) {
      if (message.data.module == this.name) {

console.log("INTENDED FOR US");
...
//console.log("EXPECTED REQUEST: " + expected_request + " --- " + message.request);
...
console.log("ABOUT TO SEND PEER REQUEST WITH FILTER");
...
console.log("WHAT ABOUT THIS GUY: " + peerfilter(this.app.network.peers[i]));
	if (peerfilter(this.app.network.peers[i])) { peers.push(this.app.network.peers[i]); }
      }
    }
    if (peers.length == 0) { return; }

console.log("REQUEST BEING SENT: " + JSON.stringify(message));
