...
//console.log(JSON.stringify(rows));
  ...
  //Updated db query with filter
  async sendPeerDatabaseRequestWithFilter(db, sql, peer, filter=null, mycallback=null) {
    
    if (!this.isSQLSafe(sql)) {return false};
    ...
    var relevant_peers = filter(peers);

    relevant_peers.forEach((peer) => {
      peer.sendRequestWithCallback(message.request, message.data, function (res) {
        JSON.stringify("callback data2: " + JSON.stringify(res));
        mycallback(res);
      });
