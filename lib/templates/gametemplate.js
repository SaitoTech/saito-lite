...
console.log("is interface locked? " + str + " -- " + force + " -- " +this.lock_interface);
...
console.log("am i locked? " + this.lock_interface);
...
console.log("CONTINUING TO RUN QUEUE: " + JSON.stringify(game_self.game.queue));
...
console.log("ack queue: " + JSON.stringify(game_self.game.queue));
        return 0;
      }
    });
    ...
    this.commands.push((game_self,gmv) => {
	  ...
	  for (let i = 0; i < players_to_go.length; i++) {
	    if (game_self.game.player == parseInt(players_to_go[i])) {
	      // only if we have no confirmed already
	      if (game_self.game.confirms_needed[i] == 1) {
console.log("player moving: " + (i+1));
...
console.log(gmv[1] + " --- " + game_self.app.wallet.returnPublicKey());
...
console.log("DONE");
console.log("CONFS NEED: " + JSON.stringify(game_self.game.confirms_needed));
...
console.log("executing ISSUEKEYS");
...
console.log("and out with queue: " + JSON.stringify(game_self.game.queue));
