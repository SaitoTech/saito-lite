        ...
        for (let i = 0; i < app.options.games.length; i++) {
          if (game_id == app.options.games[i].id) {
            ...
            if (tx.isTo(app.wallet.returnPublicKey())) {
              if (tx_processed == 0) {
console.log("STANDARD GAME MOVE");
  ...
  handleGameMove(app, tx, only_process_next_move=0) {
    ...
    //
    let txmsg = tx.returnMessage();
 console.log("HGM: txmsg step: " + txmsg.step.game + " -- " + JSON.stringify(game_self.game.step));
...
console.log("games in options: " + game_self.app.options.games.length);

      for (let i = 0; i < game_self.app.options.games.length; i++) {
console.log("checking game_id: " + game_self.app.options.games[i].id);
        if (game_id === game_self.app.options.games[i].id) {
          following_game = 1;
        }
      }
...
console.log("am i following: " + following_game);

      if (following_game == 1) {

console.log("following this game");
...
//        try {
	  ...
	  if (only_process_next_move == 1) {
	    ...
	    // unprocessed == not next move, future == for this game in future
	    if (game_self.isFutureMove(tx.transaction.from[0].add, txmsg) && game_self.isUnprocessedMove(tx.transaction.from[0].add, txmsg) == 0) {
	      if (txmsg.step.game != 0) {
	        if (game_self.game.id === txmsg.game_id) {
console.log("PUSHING INTO FUTURE: " + JSON.stringify(txmsg.turn));
...
console.log("active --> " + this.gaming_active);
	      ...
	      if (game_self.game.id === txmsg.game_id) {
console.log("PUSHING INTO FUTURE 2: " + JSON.stringify(txmsg.turn));
	        game_self.game.future.push(JSON.stringify(tx.transaction));
	      }
	      game_self.saveFutureMoves(game_self.game.id);
	      game_self.gaming_active = 0;
console.log("returning 41");
	      return;
	    }
	  } else {
...
console.log("... else ...");
	    ...
	    let is_unprocessed_move = game_self.isUnprocessedMove(tx.transaction.from[0].add, txmsg);
...
if (is_future_move == 1) { console.log("RECEIVED MOVE BUT FUTURE MOVE...: " + JSON.stringify(txmsg)); }
if (is_unprocessed_move == 1) { console.log("RECEIVED MOVE REQUIRING PROCESSING: " + JSON.stringify(txmsg)); }

	      if (game_self.game.future === undefined || game_self.game.future === "undefined" || game_self.game.future === null) { game_self.game.future = []; }

	      if (txmsg.game_id === game_self.game.id && (is_future_move == 1 || is_unprocessed_move == 1)) {
console.log("pushing move onto future stack 3!");
console.log("PUSHING INTO FUTURE 3: " + JSON.stringify(txmsg.turn));
	        game_self.game.future.push(JSON.stringify(tx.transaction));
	      }
...
console.log("continuing to process ...");
...
console.log(tx.transaction.from[0].add + " sends TXMSG: " + JSON.stringify(txmsg));
console.log("GAMESTEP: " + JSON.stringify(game_self.game.step) + " -- " + game_self.game.queue);
	  return 0;
	}
        ...
        // QUEUE //
        ///////////
console.log("about to execute in queue");
        if (game_self.game.queue != undefined) {
console.log("about to execute in queue 2");
	  // ACCEPT / JOIN / OPEN will not have turn defined
	  if (txmsg.turn != undefined) {
console.log("about to execute in queue 3");
            for (let i = 0; i < txmsg.turn.length; i++) { 
console.log("about to execute in queue 4");
...
console.log("queue done here...");
          }
        }

console.log("processing future moves 313");
	...
	//
	// we are not following this game, but if it is addressed to us and for an early step
	// we should future move it to be safe
	//
console.log("not following...");
	if (tx.isTo(this.app.wallet.returnPublicKey())) {
console.log("but is to me");
	  if (txmsg.step.game < 3) {
console.log("and is low step");

	    console.log("PUSHING INTO FUTURE 4: " + JSON.stringify(txmsg.turn));
            game_self.game.future.push(JSON.stringify(tx.transaction));
...
//    }
  ...
  isFutureMove(player, txmsg) {
    ...
    if (txmsg.step.game > (game_self.game.step.game+1)) { console.log("xa1"); return 1; }
    if (txmsg.step.game > (game_self.game.step.players[player]+1)) { console.log("xa2"); return 1; }
...
console.log("GAME IS SAVED, processing future moves...");
    this.processFutureMoves();
...
console.log("future moves processed...");
...
console.log("newcard 2: " + newcard);
              }


              newcard = game_self.app.crypto.hexToString(newcard);
console.log("newcard 3: " + newcard);
...
console.log("noping out as i am not the player to send: " + playerid + " ---- " + this.game.player);
	...
	//
        if (loop_length == game_self.game.queue.length && cont == 1) {
          if (loop_length_instruction === game_self.game.queue[game_self.game.queue.length-1]) {
console.log("into HGL");
...
console.log("PLAYER: " + game_self.game.player);
    ...
    let nothing_left = 0;
    while (game_self.game.future.length > 0 && game_self.gaming_active != 1 && game_self.game.halted != 1 && nothing_left == 0) {

console.log("... nll: " + game_self.game.future.length + " -- " + game_self.gaming_active + " -- " + game_self.game.halted + " -- " + nothing_left);
...
console.log("EXECUTING MOVE THAT ARRIVED WHILE WAITING!!!!! " + JSON.stringify(ftxmsg));
          game_self.game.future.splice(ii, 1);
          cont = this.handleGameMove(this.app, ftx, 1);
	  tx_processed = 1;
	} else {
console.log("SKIPPING!!!!! " + JSON.stringify(ftxmsg));
...
console.log("swapping our future moves into this game's future moves");
	    this.app.options.games[i].future = this.game.future;
console.log("SAVED: " + JSON.stringify(this.app.options.games[i].future));
	  }
        }
      }
    }
...
console.log("saving game_id: " + game_id);
...
console.log("B3");
          ...
          this.app.storage.saveOptions();
console.log("Saved ID: " + this.app.options.games[i].id);
//
// bizarre as it seems, this next line may be necessary to prevent a bug -- perhaps browsers caching options.games, and then not claiming to have them when relay moves come in...
//
console.log(JSON.stringify(this.app.options.games).substring(0, 50));
console.log("B4");
          return;
        }
      }
...
console.log(" and finished saving...");
...
console.log("RECEIVED MOVE BY RELAY: " + JSON.stringify(gametxmsg));
	  ...
	  //
console.log("GAMES ARRAY: " + JSON.stringify(this.app.options.games).substring(0,100));

          this.handleGameMove(this.app, gametx, 1); 
console.log("done handle game move!");
	  this.processFutureMoves();
console.log("done process future moves");
