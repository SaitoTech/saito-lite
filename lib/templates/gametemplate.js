...
console.log("about to see if we should handle missing moves...");
	    ...
	    //
	    if (txmsg.step == undefined) {
console.log("NO STEP IN TX");
	      return;
	    }
	    if (game_self.game.step == undefined) {
console.log("NO STEP in GAME");
	      return;
	    }


	    //
	    // any missing moves will be in our pending slot, otherwise
	    //
	    if (
	      (txmsg.step.game > (game_self.game.step.game+1)) ||
	      (game_self.game.halted || game_self.gaming_active)
	    ) {
...
if (game_self.gaming_active == 1) { console.log("RECEIVED MOVE BUT GAMING ACTIVE (HALTED)...: " + JSON.stringify(txmsg)); }
	  ...
	  if (txmsg.turn != undefined) {
            ...
            game_self.saveGame(game_self.game.id);
	    //
	    // Juily 4 - unsure should_we_save does anything
	    //
	...
	if (game_self.game.future) {
	  if (game_self.game.future.length > 0) {
	    for (let ii = 0; ii < game_self.game.future.length; ii++) {
	      ...
	      if (this.isUnprocessedMove(ftxmsg)) {
...
//    }
  ...
  isUnprocessedMove(txmsg) {

    let game_self = this;

    if (txmsg.step.game == (game_self.game.step.game+1) && txmsg.game_id === game_self.game.id) { return 1; }
...
console.log("ftxmsg game step ---> " + ftxmsg.step.game);
