  ...
  initializeQueueCommands() {
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "LOGDECK") {
        ...
        game_self.updateLog(`Contents of Deck: ${game_self.game.deck[deckidx].cards}`);
      }
      game_self.game.queue.splice(game_self.game.queue.length-1, 1);
      return 1;
    });
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "LOGHAND") {
        let deckidx = parseInt(gmv[1]);
        game_self.updateLog(`Contents of Hand: ${game_self.game.deck[deckidx].hand}`);
      }
      game_self.game.queue.splice(game_self.game.queue.length-1, 1);
      return 1;
    });
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "LOGPOOL") {
        ...
        game_self.updateLog(`Contents of Pool: ${game_self.game.pool[poolidx]}`);
      }
      game_self.game.queue.splice(game_self.game.queue.length-1, 1);
      return 1;
    });
    ...
    this.commands.push((game_self,gmv) => {
      if (gmv[0] === "SIMPLEDEAL") {
        ...
        let players = parseInt(gmv[1]);
        let cards = parseInt(gmv[2]);
        let deckidx = gmv[3];
        let deck = gmv[4];

        for (let i = players; i >= 1; i--) {
          game_self.game.queue.push("DEAL\t"+deckidx+"\t"+i);
	}
        for (let i = players; i >= 1; i--) {
        ...
        game_self.game.queue.push("DECK\t"+deckidx+"\t"+deck);

console.log("QUEUE: "+game_self.game.queue);
...
console.log("MOVE IS: " + player + " / " + hash + " / " + sig + " and i am p" + game_self.game.player);
...
console.log("RECEIVED: " + players + " --- " + hashes);
...
console.log("the initiating player is: "+initiating_player);
...
console.log("do we have all players: " + do_we_have_all_players + " and am I done? " + game_self.game.sroll_done);
console.log("i am player: " + game_self.game.player + " and initiating player is " + initiating_player);
...
console.log("we do not have all players, exiting...");
	  return 0;
	}
...
console.log("everyone has contributed, except originating player has not sent twice...");
...
console.log("everyone has contributed, and originating player has sent twice...");
...
console.log("entering DECKENCRYPT");

        let deckidx = parseInt(gmv[1]);
        let playerid = parseInt(gmv[2]);

console.log("entering DECKENCRYPT idx" + deckidx);
...
console.log("WE HAVE SENT MESSAGE");
...
console.log("HERE WEARE IN CRYPTOKEY");
...
console.log("REMOVING and continuing!");
