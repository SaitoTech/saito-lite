...
class GameTemplate extends ModTemplate {
  ...
  onConnectionStable() {
/**** July 23 ****
    console.log("the connection is up... and it was down...");
    //
    // so we should...
    //
    // 1. resend any pending txs (auto-done?)
    // 2. execute future moves
    //
    this.loadGame(); // load the latest game and run its queue

    let thismodule = this.game.module;
    let actmod = this.app.modules.returnModule(thismodule);
    if (actmod == null) { return; }
    if (actmod.browser_active == 1 && this.name == thismodule) {
console.log(thismodule + " -- " + actmod.browser_active);
console.log("LOADING AND RUNNING QUEUE");
      this.runQueue();
        ...
        for (let i = 0; i < app.options.games.length; i++) {
          if (game_id == app.options.games[i].id) {
            ...
            if (tx.isTo(app.wallet.returnPublicKey())) {
              if (tx_processed == 0) {
		console.log("BLOCK: " + game_self.browser_active + " -- " + JSON.stringify(txmsg.turn));
		if (game_self.browser_active || game_self.game.initializing == 1) {
                  game_self.handleGameMove(app, tx);
		} else {
	          console.log("PUSHING INTO FUTURE 35: " + JSON.stringify(txmsg.turn));
		  if (!game_self.game.future.includes(JSON.stringify(tx.transaction))) {
...
console.log("updating game step to: " + game_self.game.step.game);
	  }
	} else {
	  return 0;
	}
...
console.log("saving with game id: " + game_self.game.id);
            game_self.saveGame(game_self.game.id);
	    //
...
console.log("X CONT ON EXIT IS: " + cont);
	  }
	}

console.log("cont x is: " + cont);

	// nothing has been removed from this queue, so pointless to continue
...
console.log("SAVED!");
  ...
  loadGame(game_id = null) {
    ...
    if (game_id != null) {
console.log("my game id is: " + this.game.id);
console.log("loading other game: " + game_id);
      for (let i = 0; i < this.app.options.games.length; i++) {
console.log("i: "+i);
        if (this.app.options.games[i].id === game_id) {
          this.game = JSON.parse(JSON.stringify(this.app.options.games[i]));
console.log("about to return the game! ");
          return this.game;
        }
      }
    }
...
console.log(" ... sent on chain ");
  ...
  async handlePeerRequest(app, message, peer, mycallback = null) {
    ...
    if (message.request === "game relay gamemove") {
	  ...
	  //
	  if (game_self.browser_active || game_self.game.initializing == 1) {
            game_self.handleGameMove(game_self.app, gametx, 1); 
	    game_self.processFutureMoves();
	  } else {
