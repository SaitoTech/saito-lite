...
console.log("received move for game: " + txmsg.game_id);
	...
	if (game_self.game.id != txmsg.game_id) {
	  //
	  // only backup if the old game had no steps (i.e. we might receive a shuffle)
	  //
	  if (old_game_id.game.step.game == 0) {
...
console.log("standard game move... " + txmsg.game_id);
      ...
      if (app.options.games) {
        for (let i = 0; i < app.options.games.length; i++) {
          if (game_id == app.options.games[i].id) {
            //
            // standardgame move -- this breaks observer mode but prevents servers
            //
            if (tx.isTo(app.wallet.returnPublicKey())) {
              if (tx_processed == 0) {
console.log("moving into handle game move with tx for game move");
      ...
      //
      if (old_game_id != "") {
console.log("restoring old / active game");
	//
  ...
  handleGameMove(app, tx, only_process_next_move=0) {

    let game_self = this;

    //
    // we are processing this move! we need to reset this variable 
    // to 0 once the move has fully executed (i.e. runQueue) has 
    // completed and we have saved / not-saved the game state as 
    // necessary.
    //
...
console.log("we are following this game!");
...
//        try {
	  ...
	  if (only_process_next_move == 1) {

console.log("only processing next move...");

	    if (game_self.isFutureMove(tx.transaction.from[0].add, txmsg)) {
	      if (txmsg.step.game != 0) {

console.log("txmsg step (game): " + txmsg.step.game + " versus game-game-step-game: " + game_self.game.step.game);

	        console.log("ERROR 581029: game moves received in the wrong order. Possible relay errors");
...
console.log("GAME IS HALTED, EVEN THOUGH THIS IS LEGIT MOVE, PAUSING...");
...
console.log("active --> " + this.gaming_active);
	      ...
	      if (game_self.game.id === txmsg.game_id) {
	        game_self.game.future.push(JSON.stringify(tx.transaction));
	      }
	      game_self.saveGame(game_self.game.id);
	      game_self.gaming_active = 0;
	      return;
...
console.log("about to see if we should handle missing moves...");

	    //
	    // no missing moves if the game does not exist
	    //
	    if (txmsg.step == undefined) { return; }
	    if (game_self.game.step == undefined) { return; }

console.log("about to see if we should handle missing moves 2...");

	    let is_future_move = game_self.isFutureMove(tx.transaction.from[0].add, txmsg);

console.log("and out");
console.log("is future move? " + is_future_move);
console.log(" and halted: " + (game_self.game.halted ) );
console.log(" and active: " + (game_self.gaming_active ) );
console.log("and here we go");

	    let is_the_game_halted = 0;
	    if (game_self.game.halted == 1)   { is_the_game_halted = 1; }
	    if (game_self.gaming_active == 1) { is_the_game_halted = 1; }
	    if (is_future_move == 1)          { is_the_game_halted = 1; }

console.log("about to test our if clause with: " + is_the_game_halted);if (is_the_game_halted == 1) {

console.log("AND THROUGH");
console.log("if step 1: " + game_self.gaming_active + "? -- " + game_self.game.halted + " vvvv " + txmsg.step.game + " / " + game_self.game.step.game);;
if (game_self.game.halted == 1) { console.log("RECEIVED MOVE BUT GAMING HALTED...: " + JSON.stringify(txmsg)); }
if (game_self.gaming_active == 1) { console.log("RECEIVED MOVE BUT GAMING ACTIVE (HALTED)...: " + JSON.stringify(txmsg)); }
	      ...
	      for (let c = 0; c < app.wallet.wallet.pending.length; c++) {
                let newtx = new saito.transaction(JSON.parse(app.wallet.wallet.pending[c]));

	        if (newtx.msg) {
	          if (newtx.msg.step) {
	            if (newtx.msg.step.game == (game_self.game.step.game+1)) {
                      //
                      // remove pending tx
                      //
	              app.wallet.wallet.pending.splice(c, 1);

	              //
	              // process this one instead
	              //
                      game_self.handleGameMove(app, newtx, 1);
...
console.log("we hit here instead");
            }
console.log("or here 2412");
	  }
...
console.log("maybe we hit here...");

	//
	// check if this is an unprocessed move...
	//
        if (txmsg.step == undefined) { txmsg.step = {}; }
        if (txmsg.turn == undefined) { txmsg.turn = []; }
	if (this.isUnprocessedMove(tx.transaction.from[0].add, txmsg)) {
console.log("this is an unprocessed tx... 1");
console.log("tx add: " + tx.transaction.from[0].add);
console.log("step : " + JSON.stringify(game_self.game.step));
console.log("txmsg: " + JSON.stringify(txmsg));
	  game_self.game.step.players[tx.transaction.from[0].add] = txmsg.step.game;
console.log("this is an unprocessed tx... 2");
          if (txmsg.step.game > game_self.game.step.game) {
console.log("this is an unprocessed tx... 3");
	    game_self.game.step.game = txmsg.step.game;
	  }
console.log("this is an unprocessed tx... 4");
	} else {
console.log("We have received a move in handleGameMove, but are being told that it is processed or old... ignoring");
	  return 0;
	}

console.log("About to enter queue...");
	  ...
	  if (txmsg.turn != undefined) {
            for (let i = 0; i < txmsg.turn.length; i++) { 
  	      game_self.game.queue.push(txmsg.turn[i]); 
	    }
            game_self.saveGame(game_self.game.id);
	    //
	    // July 4 - unsure should_we_save does anything
	    //
            let should_we_save = game_self.runQueue(txmsg);
          }
        }
	...
	//
	// we have just processed a move, so lets see if we have the next move in our future stack, if so, execute it recursively
	//
	if (game_self.game.future) {
	  if (game_self.game.future.length > 0) {
	    for (let ii = 0; ii < game_self.game.future.length; ii++) {
...
console.log("^^^^^^^^^^^^^^^^^^^^^^^^^");
console.log("EXECUTE FUTURE STACK MOVE");
console.log("^^^^^^^^^^^^^^^^^^^^^^^^^");
	        // no need for this as sending directly --> game_self.gaming_active = 0; // so next move will execute
                this.handleGameMove(app, ftx, 1);
...
//    }
    ...
    //
    // be OK with other moves arriving now
    //
    game_self.gaming_active = 0;
...
console.log("TXMSG: " + JSON.stringify(txmsg));
console.log("GAMESTEP: " + JSON.stringify(game_self.game.step));

console.log("a");
    if (txmsg.game_id !== game_self.game.id) { return 0; }
console.log("b");
console.log("txmsg step game: " + txmsg.step.game);
console.log("game self game step game " + game_self.game.step.game);
console.log("does this compare: " + (parseInt(txmsg.step.game) <= (parseInt(game_self.game.step.game)+1)));
console.log(" and in!");
    if (parseInt(txmsg.step.game) <= (parseInt(game_self.game.step.game)+1)) { 
      console.log("execution reaches here... this is not a valid move");
      return 0; 
    }
console.log("c");
    if (txmsg.step.game <= (game_self.game.step.players[player]+1)) { return 0; }
console.log("d");
...
console.log("TXSMSG STEP : " + txmsg.step.game);
...
console.log("HERE IS THE TX INADEQUATE w/ INFO: ");
console.log(JSON.stringify(txmsg));
    ...
    if (this.game.players_set == 0) {
      ...
      //
      for (let i = 0; i < this.game.players.length; i++) {
console.log("Updating Step for : " + players[i]);
...
console.log("RECEIVE ACCEPT REQUEST: " + JSON.stringify(this.game.queue));
    ...
    if (this.initializeGameFeeder(game_id) == 1) {
      //
      // ensure options up-to-date -- added here as loadGame made conditional inside -- now not saving to see
      //
console.log("SAVING GAME 1: " + game_id);
      this.saveGame(game_id);
    } else {
console.log("SAVING GAME 2: " + game_id);
      this.saveGame(game_id);
    }
...
console.log("FUTURE MOVES 2: " + this.game.future.length);
for (let i = 0; i < this.game.future.length; i++) {
  console.log(i + " -- " + this.game.future[i]);
}
    ...
    //
    //
console.log("INITIALIZE FEEDER IN INITIALIZE REQUEST: " + JSON.stringify(this.game.queue));
    //
...
console.log("INIT GAME FEEDER QUEUE PRE INIT: " + JSON.stringify(this.game.queue));
    ...
    //
    let pending_tx_is_accept = -1;

    for (let i = 0; i < this.app.wallet.wallet.pending.length; i++) {
      let tmptx = new saito.transaction(JSON.parse(this.app.wallet.wallet.pending[i]));
      try {
        ...
        if (txmsg.game_id != "") {
  	    ...
  	    //
	    // changed to != Sun Mar 15, why == before? TODO check
	    //
		...
		//
		// avoid rebroadcast on init because relay is faster than onchain
		// - take it on faith that our accept tx will make it if we ahvec
		// been able to join the game through the arcade.
		//
	        if (pending_tx_is_accept == -1 && txmsg.request == "accept") {
	          pending_tx_is_accept = 1;
	        } else {
		  pending_tx_is_accept = 0;
	        }
                ...
                if (game_self == this) {
                  if (game_self.game.step.game > txmsg.step.game) {
                    ...
                    this.updateStatus("Rebroadcasting our last move to be sure opponent receives it. Please reload in 15 seconds.");
                    this.updateLog("we just rebroadcast our last move to be sure opponent receives it. Please reload in 15 seconds.");

		    this.app.network.propagateTransaction(tmptx);

		    //
	            // auto-reload if this game is active on-screen
	            //
...
console.log("error rebroadcasting pending tx: " + err);
      }
    }

console.log("NOW WE RUN OUR QUEUE");

    this.runQueue();

console.log("JUST DONE RUNQUEUE: ");
console.log("halted --> " + this.game.halted);
console.log("active --> " + this.gaming_active);



console.log("RUN QUEUE IS DONE -- do we have future moves? ");
if (this.game) {
  if (this.game.future) {
    if (this.game.future.length > 0) {
console.log("WE HAVE FUTURE MOVES, SO PROCESS THEM!!!");
      this.processFutureMoves();
    }
    console.log(this.game.future.length);
  } else {
    console.log("no game future");
  }
} else {
    console.log("no game");
}
...
console.log("resflip... " + deckidx + " - " + cardnum + " - " + poolidx);
          ...
          //
console.log("REMOVING FROM QUEUE: " + (gqe-cardnum) + " - " + cardnum);
          game_self.game.queue.splice(gqe-cardnum, cardnum);
console.log("Q: " + game_self.game.queue);
...
//alert("Process Future Moves: " + game_self.game.id);

    //
    // do we have future game moves waiting
    //
console.log("ABOUT TO STOP: do we have future moves? " + game_self.game.future.length);
console.log("gaming active --> " + game_self.gaming_active);
console.log("game halted   --> " + game_self.game.halted);
console.log("game step --> " + game_self.game.step.game);


console.log("NUM FUTURE: " + game_self.game.future.length);

    if (game_self.game.future.length > 0 && game_self.gaming_active != 1 && game_self.game.halted != 1) {
      for (let ii = 0; ii < game_self.game.future.length; ii++) {
        let ftx = new saito.transaction(JSON.parse(game_self.game.future[ii]));
        let ftxmsg = ftx.returnMessage();
//console.log("generated future move...: " + JSON.stringify(ftxmsg.turn));
console.log("Should We Execute? ");
console.log("ftxmsg game step ---> " + ftxmsg.step);
console.log("next game step ---> " + game_self.game.step.game+1);
console.log("ftxmsg game id ---> " + ftxmsg.game_id);
console.log("game self id ----> " + game_self.game.id);
        if (ftxmsg.step.game == (game_self.game.step.game+1) && ftxmsg.game_id == game_self.game.id) {
// save game added July 4

console.log("INCREMENT STEP AND SAVE GAME");

	  game_self.game.step.game = ftxmsg.step.game;
          game_self.saveGame(game_self.game.id);
          game_self.game.future.splice(ii, 1);

console.log("EXECUTING MOVE THAT ARRIVED WHILE WAITING!!!!! " + JSON.stringify(ftx));
          cont = this.handleGameMove(this.app, ftx, 1);
	} else {
console.log("NOT EXECUTING MOVE");
	  if (ftxmsg.step.game < game_self.game.step.game) {
console.log("purging future move... ");
            game_self.game.future.splice(ii, 1);
	  }
	}
      }
      //
      //
      // do not save exiting as splice has removed commands
      //game_self.saveGame(game_self.game.id);
    }
...
console.log("saving game A");
console.log("SAVED QUEUE: " + JSON.stringify(this.game.queue));
...
console.log("actually saving game!");
    this.app.storage.saveOptions();
console.log("finished saving the game!");
...
console.log("RECEIVED MOVE BY RELAY: " + JSON.stringify(gametxmsg));
  
	  //
	  // relay moves are fine, but they can come out-of-order
	  //
	  // so we will not process out-of-order relayed moves
	  // unless we are at step 1. The third argument here
	  // tells handleGameMove to bonk-out if relay messages
	  // are not absolutely next.
	  //
          this.handleGameMove(this.app, gametx, 1); 
