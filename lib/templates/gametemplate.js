...
class GameTemplate extends ModTemplate {
  ...
  ***REMOVED***
  ...
  displayModal(modalHeaderText, modalBodyText="") {
    salert(`${modalHeaderText***REMOVED***: ${modalBodyText***REMOVED***`);
  ***REMOVED***
...
***REMOVED***
  ...
  ***REMOVED***
  ...
  receiveInviteRequest(blk, tx, conf, app) {
    ...
    if (txmsg.options != "") { options = txmsg.options; ***REMOVED***
    if (txmsg.invite_sig != "") { invite_sig = txmsg.invite_sig; ***REMOVED***
    if (txmsg.players_needed > 1) { players_needed = txmsg.players_needed; ***REMOVED***
...
console.log("\n\nSWAMMPING: " + this.game.options + " with " + options);
***REMOVED***
***REMOVED***
      //
...
***REMOVED***
    ...
    //
console.log("PLAYERS NEEDED: " + this.game.players_needed + " ---- " + this.game.players.length);
    if (this.game.players_needed > this.game.players.length) {
console.log("NOT ENOUGH PLAYERS YET! " + this.game.players_needed + " needed, only " + tx.transaction.to.length + " are here");
...
console.log("NOT EVERYONE HAS ACCEPTED");
      return;
***REMOVED*** 

console.log("EVERYONE HAS ACCEPTED!");
...
console.log("SAVING GAME ID HERE: " + game_id);
      this.saveGame(game_id);

***REMOVED***

console.log("MOVING INTO IGF with ID: " + game_id);

    //
    // if we hit this point, everyone has accepted the game
    // so we can move into handleGame
    //
console.log("EVERYONE HAS ACCEPTED, so move into FEEDER: ");
...
console.log("INITIALIZE GAME IS DONE!");
...
***REMOVED*** FLIPCARD [decknum] [cardnum] [poolnum] [player]
...
console.log("ENDING WITH WHAT QUEUE?: " + game_self.game.queue);
          game_self.saveGame(game_self.game.id);
...
***REMOVED***
  ...
  ***REMOVED***
    ...
    ***REMOVED***
        ...
        if (gmv[0] === "FLIPRESET") {
          let poolidx  = gmv[1];
console.log("A -- FLIPRESET " + poolidx);
          while (game_self.game.pool.length < poolidx) { 
console.log("B -- FLIPRESET");
	    game_self.addPool();
	  ***REMOVED***
console.log("C -- FLIPRESET");
          game_self.game.pool[poolidx-1].crypt = [];
console.log("D -- FLIPRESET");
          game_self.game.pool[poolidx-1].keys  = [];
console.log("E -- FLIPRESET");
          game_self.game.pool[poolidx-1].decrypted = 0;
console.log("F -- FLIPRESET");
          game_self.game.queue.splice(gqe, 1);
console.log("G -- FLIPRESET POST QUEUE: " + JSON.stringify(game_self.game.queue));
    ***REMOVED***
...
console.log("not my turn yt...");
  ...
  ***REMOVED***
	...
	***REMOVED***
	...
	if (cont == 1) {
console.log("BEFORE GAME LOOP: " + cont);
	  cont = this.handleGameLoop();
console.log("AFTER GAME LOOP: " + cont);
	***REMOVED***


	if (cont == 0) {
console.log("RETURNING bc CONT is zero");
	  return;
	***REMOVED***
  ***REMOVED***
***REMOVED***
