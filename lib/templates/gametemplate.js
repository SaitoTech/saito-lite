...
console.log(" ... into onConfirmation in the game");
...
console.log("THIS GAME MOVE IS FOR US!");
              if (tx_processed == 0) {
...
console.log("ARE WE OBSERVING THIS GAME: " + app.options.games[i].observer_mode);

	      // we could be observing this game
              if (app.options.games[i].observer_mode == 1) {

console.log("OBSERVER MODE ACTIVE IS ZERO: " );

console.log("txmsg: " + JSON.stringify(txmsg));
console.log("txmsg: " + JSON.stringify(txmsg.step));

		if (app.options.games[i].observer_mode_active == 0) {
		  app.options.games[i].observer_mode_active = 1;
		  app.options.games[i] = txmsg.game_state;
// try this --- force minus 1
console.log("setting step to: " + parseInt(txmsg.step.game)-1);
		  app.options.games[i].step.game = parseInt(txmsg.step.game)-1;
		  app.storage.saveOptions();
		  game_self.game.step.game = app.options.games[i].step.game;
		}
...
console.log(" txmsg step -> " + txmsg.step.game );
...
console.log("GAME MOVE: " + game_self.game.queue[gqe]);
        ...
        // core game engine
        //
        // SECUREROLL_COMMIT [player] [random] [move]
        // SECUREROLL_RESPONSE [player] [random]
        // SECUREROLL_REQUEST [player] [mystery hash]
        // SHUFFLE [decknum]
	...
	//
	  ...
	  if (gmv[1]) {
        ...
        if (gmv[0] === "OBSERVER") {
          ...
          let msg = this.app.crypto.stringToBase64(JSON.stringify(msgobj));
try {
	  game_self.updateLog(`Player ${game_self.game.player} has enabled observer mode. This can leak data on your private hand to your opponent. Observers can watch the game from your perspective using this link: <p></p><div style="margin-left:15px;margin-right:15px;margin-bottom:15px;">${window.location.origin}/arcade/?i=watch&msg=${msg}</div>`);
} catch (err) {}
...
console.log("I AM NOT SENDER: " + sender);
	  }
