  ...
  handleGameMove(app, tx, only_process_next_move=0) {
    ...
    try {
	  ...
	  if (only_process_next_move == 1) {
	    ...
	    //
	    if (txmsg.step.game > (game_self.game.step.game+1)) {
	      if (!game_self.game.future) { game_self.game.future = []; }
...
for (let c = 0; c < app.wallet.wallet.pending.length; c++) {
	     ...
	     if (newtx.transaction.msg.step.game == (game_self.game.step.game+1)) {
               //
               // remove pending tx
               //
...
console.log("ERROR: " + JSON.stringify(err));
  ...
  createSorryAcceptedTransaction(recipient, game_id) {
    ...
    let tx = this.app.wallet.createUnsignedTransactionWithDefaultFee(recipient);
        ...
        tx.transaction.msg.request      = "sorry";
  ...
  createAcceptTransaction(game) {
    ...
    for (let i = 0; i < players.length; i++) {
      tx.transaction.to.push(new saito.slip(players[i], 0.0));
        ...
        tx.transaction.msg.game_id        = id;
        tx.transaction.msg.options        = options;
        tx.transaction.msg.players        = players;
        tx.transaction.msg.players_needed = players_needed;
        tx.transaction.msg.players_sigs   = [...players_sigs, this.app.crypto.signMessage(`invite_game_${ts}`, this.app.wallet.returnPrivateKey())];
	//
...
console.log("noping out as i am not the player to send: " + playerid + " ---- " + this.game.player);
  ...
  saveGame(game_id) {
    ...
    //usure this is the right solution
  ...
  sendMessage(type = "game", extra = {}, mycallback = null) {
    ...
    if (this.time.last_sent > (this.time.last_received+1000)) {
      ...
      let time_left = this.clock_timer - this.game.clock_spent;
      this.clock.displayTime(time_left);
      console.log("CLOCK: " + this.game.clock_limit + " -- " + this.game.clock_spent + " --- " + time_left);
      this.stopClock();
    }
...
if (this.game.id.length < 25 && this.game.players.length > 1) {
console.log("\n\nPLEASE REPORT THIS TO SAITO TEAM\nSENDING MESSAGE WITH BAD GAME ID");
process.exit();
}
    ...
    if (this.relay_moves_offchain_if_possible && send_onchain_only == 0) {
      ...
      //
      if (newtx.transaction.msg.request === "game" ||  this.initialize_game_offchain_if_possible == 1) {
...
console.log("useHUD? " + this.useHUD);
  ...
  startClock() {
    ...
    this.clock_timer = setInterval(() => {
      ...
      let time_on_clock = this.clock_timer - (x - this.time.last_received) - this.game.clock_spent;
