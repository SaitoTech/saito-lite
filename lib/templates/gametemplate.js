...
***REMOVED*** 2 = text hud
  ...
  ***REMOVED***
  ...
  initializeHTML(app) {
    ...
    GameCardbox.render(app, this.game);
  ...
  ***REMOVED***
  ...
  attachEvents(app) {
    ...
    GameCardbox.attachEvents(app, this.game);
  ***REMOVED***
...
console.log("TURN IS WHAT: " + game_self.game.step.game + " -- " + txmsg.step.game);
...
console.log("SAVED FOR REFRESH FROM HERE ON RELOAD!");
              game_self.runQueue(txmsg);
console.log("run queue 2 is done! " + JSON.stringify(this.game.queue));
        ***REMOVED***
  ...
  ***REMOVED***
      ...
      if (old_game_id != "") {
	//
	// if an older game exists, save new before we load the old
	//
        game_self.saveGame(game_self.game.id);
...
***REMOVED***
    ...
    //
    //if (this.saveGameState == 1) { mymsg.saveGameState = this.returnGameState(); ***REMOVED***
    //if (this.saveKeyState == 1) { mymsg.saveKeyState = this.returnKeyState(); ***REMOVED***
