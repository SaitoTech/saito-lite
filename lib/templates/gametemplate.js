...
console.log("SK: " + (this.game.id+"_sharekey") + " -- " + sharekey);
    for (let i = 0; i < game_clone.deck.length; i++) {
      if (sharekey) {
        game_clone.deck[i] = this.app.crypto.encodeXOR(this.app.crypto.stringToHex(JSON.stringify(game_clone.deck[i])), sharekey);
      } else {
