...
console.log("GAMESELF FOR MOD: " + txmsg.module);
    let game_self = app.modules.returnModule(txmsg.module);
console.log("GAME ID FOR TX IS: " + txmsg.game_id);
...
console.log("GAME ID: " + game_self.game.id);
      }
...
console.log("and load our game 1!");
	  game_self.loadGame(txmsg.game_id);
	}
      } else {
	if (game_self.game.id != txmsg.game_id) {
console.log("and load our game 2!");
	  game_self.loadGame(txmsg.game_id);
	}
      }

console.log("GAMESTEP 1: " + JSON.stringify(game_self.game.step));
      ...
      if (tx.isTo(app.wallet.returnPublicKey()) && txmsg.request == "accept") {
        ...
        if (game_self.receiveAcceptRequest(blk, tx, conf, app) == 0) { 
          ...
          //
          if (old_game_id != "") {
console.log("restoring old game ID...");
...
console.log("GAMESTEP 2: " + JSON.stringify(game_self.game.step));
      ...
      if (app.options.games) {
        for (let i = 0; i < app.options.games.length; i++) {
          if (game_id == app.options.games[i].id) {
            ...
            //
            if (tx_processed == 0) {
console.log("handling game move...");
console.log("game id: " + game_self.game.id);
              game_self.handleGameMove(app, tx);
console.log("done handling game move...");
            }
          }
        }
      }
  ...
  handleGameMove(app, tx, only_process_next_move=0) {
    ...
    //
    // if game does not exist, load it
    //
    if (!game_self.game) {
console.log("GAME DOES NOT EXIST, CREATE");      
    }
...
console.log("... 3");
	    ...
	    //
console.log("TX STEP: " + txmsg.step.game);
try {
console.log("MY STEP: " + game_self.game.step.game);
} catch (err) {
  console.log("ERROR WITH GAME: ");
  console.log(JSON.stringify(game_self));
}

	    if (txmsg.step.game > (game_self.game.step.game+1)) {
console.log("FUT: " + game_self.game.future);
...
//    	              }
	              ...
	              // and exit
	              //
console.log("and exit 1");
	              return;
	            }
console.log("and exit 2");
                    return;
	          } // newtx msg
	        } // newtx msg game
	      }
console.log("and exit 3");
  	      return;
	    }
console.log("... 7");
	  }
...
console.log("TX MSG: " + JSON.stringify(txmsg));
...
console.log("useHUD? " + this.useHUD);
