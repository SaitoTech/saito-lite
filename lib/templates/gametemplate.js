...
console.log("and done with: " + accept_game);
      }
...
console.log("!!!!!!!!!!!!!!!!!!!!");
  ...
  async initializeGameFeeder(game_id) {
    ...
    if (this.game.step < 2) { this.saveGame(this.game.id); }
    this.startQueue();
    this.gaming_active = 0;
    return 1;
...
console.log("game is halted: " + game_self.game.halted + " ---- " + game_self.gaming_active);
...
//console.log("PUSHING TURN ONTO QUEUE: " + JSON.stringify(gametxmsg.turn));
  ...
  processFutureMoves() {
    ...
    //
    // clean up outdated future moves
    //
/**** DEC 16 *** ADDED IN DEBUGGING, REMOVING TO AVOID ISSUES
    for (let i = 0; i < game_self.game.future.length; i++) {
      let queued_tx = new saito.transaction(JSON.parse(game_self.game.future[i]));
      let queued_txmsg = queued_tx.returnMessage();
      if (queued_txmsg.step.game <= game_self.game.step.game && queued_txmsg.step.game <= game_self.game.step.players[queued_tx.transaction.from[0].add]) {
...
//
// CLOCK SUPPORT FROM RELAY SERVER - 3rd party
//
//console.log("TX: " + JSON.stringify(message.data));
...
//
// having this seems to avoid timeout issues accessing local game
// save may still be underway...
//
console.log("TXMSG: " + JSON.stringify(gametxmsg));
	  ...
	  // else added FEB 11 to avoid duplicates
          } else {
...
try {
  ...
  addPublickeyConfirm(pubkey, confs) {
    this.game.confirms_received += parseInt(confs);
    this.game.confirms_players.push(pubkey);
  }
...
console.log("DECK: " + JSON.stringify(game_self.game.deck));
    ...
    this.commands.push((game_self,gmv) => {
      if (gmv[0] === "SEND") {
        ...
        let sender = JSON.stringify(gmv[1]);
        let receiver = JSON.stringify(gmv[2]);
        let amount = JSON.stringify(gmv[3]);
        let ts = gmv[4];
        ...
        game_self.app.wallet.sendPayment(sender, receiver, amount, ts, function(robj) {
    ...
    this.commands.push((game_self,gmv) => {
      if (gmv[0] === "RECEIVE") {
        ...
        game_self.app.wallet.receivePayment(sender, receiver, amount, ts, function(robj) {
