...
console.log("deleting a bad game entry!");
...
console.log(txmsg.step.game + " --- " + game_self.game.step.game + " --- " + JSON.stringify(txmsg));
	        return; 
	      }
	    }
	  } else {
...
alert("future move bug, congrats on triggering - lets see if this is fixed!");
console.log("######################");
console.log("future move bug -- pushed to future stack, waiting for proper onchain move");
console.log("######################");
  	      return;
	    }
	...
	if (game_self.game.future) {
	  if (game_self.game.future.length > 0) {
	    for (let ii = 0; ii < game_self.game.future.length; ii++) {
	      ...
	      let tx = new saito.transaction(JSON.parse(game_self.game.future[ii]));
	      let txmsg = tx.returnMesage();
	      if (txmsg.step.game == (game_self.game.step.game+1) && txmsg.game_id == game_self.game.id) {
...
console.log("HERE: " + gamewidth + " -- " + gameheight + " --- " + this.gameboardWidth);
...
console.log("\n\n\nERROR IS HAPPENING HERE: " + game_clone.deck.length);
//console.log(JSON.stringify(game_clone.deck));
