...
class GameTemplate extends ModTemplate {
  ...
  constructor(app) {
    ...
    //this.hammer		= new GameHammerMobile();
    //this.sizer		= new GameBoardSizer();
    this.hammer         = require('../saito/ui/game-board-sizer/game-board-sizer');
    this.sizer          = require('../saito/ui/game-hammer-mobile/game-hammer-mobile');
...
console.log("SETTING ENCRYPTED KEY STATE");
...
console.log("D: " + i);
	    ...
	    if (game_self.game.players[i] === gametx.transaction.from[0].add) {
          ...
          if (sharekey) {
console.log("DECRYPTING THE DECK FOR UPDATE");
            for (let i = 0; i < gametxmsg.game_state.deck.length; i++) {
console.log("DECK NUMBER: " + i);
//console.log("DECODED: " + this.app.crypto.decodeXOR(gametxmsg.game_state.deck[i], sharekey));
//console.log("AS STR: " + this.app.crypto.hexToString(this.app.crypto.decodeXOR(gametxmsg.game_state.deck[i], sharekey)));

console.log("PDs: " + game_self.game.player_decks.length);
console.log("PD idx: " + player_idx);
	      while (game_self.game.player_decks.length <= player_idx) {
console.log("pushing back ... 1");
	             game_self.game.player_decks.push({});
	             game_self.game.player_decks[game_self.game.player_decks.length-1].deck = [];
	      }
console.log(JSON.stringify(game_self.game.player_decks[player_idx].deck));
