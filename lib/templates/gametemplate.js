	...
	game_self.game = game_self.loadGame(txmsg.game_id);
      ...
      if (app.options.games) {
        for (let i = 0; i < app.options.games.length; i++) {
		  ...
		  console.log("PROCESSING GAME MOVE in BLOCK: " + game_self.browser_active + " -- " + JSON.stringify(txmsg.turn));
                  game_self.handleGameMove(app, tx, 1);
                  game_self.processFutureMoves();
	        } else {
		  if (game_self.isFutureMove(tx.transaction.from[0].add, txmsg) || game_self.isUnprocessedMove(tx.transaction.from[0].add, txmsg)) {
		    console.log("FUTURE MOVE in BLOCK: " + game_self.browser_active + " -- " + JSON.stringify(txmsg.turn));
              	    game_self.addFutureMove(tx);
            	  } else {
		    console.log("ignoring game move in block");
		  }
            	}
              }
            }
...
console.log(" ... sent on chain ");
  ...
  async handlePeerRequest(app, message, peer, mycallback = null) {
    ...
    if (message.request === "game relay gamemove") {
	  ...
	  //
	  if (game_self.browser_active || game_self.game.initializing == 1) {
console.log("Peer Request: handle game move");
            game_self.handleGameMove(game_self.app, gametx, 1); 
	    game_self.processFutureMoves();
	  } else {
	    if (game_self.isFutureMove(gametx.transaction.from[0].add, gametxmsg) || game_self.isUnprocessedMove(gametx.transaction.from[0].add, gametxmsg)) {
console.log("Peer Request: future move");
  	      game_self.addFutureMove(gametx);
	    } else {
console.log("Peer Request: ignored");
	    }
	  }
        }
      }
