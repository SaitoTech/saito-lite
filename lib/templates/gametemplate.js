...
console.log("IN QUEUE WITH: " + gmv[0]);
...
console.log("is anyone left to move: " + anyone_left_to_move);
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "LOGDICE") {
	game_self.updateLog("Dice Roll: " + this.rollDice(6));
        game_self.game.queue.splice(game_self.game.queue.length-1, 1);
      }
      return 1;
    });
    ...
    this.commands.push((game_self, gmv) => {
      if (gmv[0] === "LOGPICK") {
        for (let i = 0; i < game_self.game.players.length; i++) {
	  game_self.updateLog(`Simultaneous Picks: ${game_self.game.state.sp[i]}`);
        }
        game_self.game.queue.splice(game_self.game.queue.length-1, 1);
      }
      return 1;
    ...
    this.commands.push((game_self,gmv) => {
	  ...
	  // reset to zero, in case next command is another flip
	...
	//
	// hashes will be on the blockchain in reverse order
	//
	  ...
	  for (let k = 0; k < players.length; k++) {
		...
		if (player_hash_idx == 2) {
		  ...
		  if (player_hash != hashes[k]) {
		    ...
		    if (!game_self.game.state) { game_self.game.state = game_self.returnState(); }
...
console.log("SIMULTANEOUS PICKS: " + JSON.stringify(game_self.game.state.sp));
	...
	let cards_to_flip = parseInt(gmv[1]);
        ...
        for (let z = 0; z < cards_to_flip; z++) {
          for (let i = this.game.players.length - 1; i >= 0; i--) {
            this.game.queue.push("FLIPCARD\t"+deckidx+"\t"+"1"+"\t"+poolidx+"\t" + (i + 1));
          }
        }
...
console.log("SENDING FROM: " + sender_crypto_address + " to " + receiver_crypto_address);
  ...
  //
  // should be over-written by game, exists here as Simultaneous Pick
  // writes to the game state so that gaming using this functionality
  // save the picks.
  //
  returnState() {
    return {};
