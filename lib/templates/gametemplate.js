...
console.log("strange unterminous step...");
...
console.log("noping out as i am not the player to send: " + playerid + " ---- " + this.game.player);
	...
	if (cont == 1) {
	  ...
	  if (cont == 0) {
	    ...
	    //
console.log("WAITING ON FUTURE MOVES? " + game_self.game.future.length);
...
console.log("game self id ----> " + game_self.game.id);
                if (ftxmsg.step.game == (game_self.game.step.game+1) && ftxmsg.game_id == game_self.game.id) {
// save our game before we execute this move, so that we do not need to repeat this
//
// added July 4, as we otherwise had a save and then future moves building up, and would often revert back to the beginning.
// this should approximate our strategy of saving BEFORE every move. And clear our the FUTURE moves in the process....
//
game_self.saveGame(game_self.game.id);
...
console.log("EXECUTING MOVE THAT ARRIVED WHILE WAITING!!!!! " + JSON.stringify(ftx));
                  game_self.game.future.splice(ii, 1);
                  this.handleGameMove(this.app, ftx, 1);
	        } else {
console.log("in else bucket...");
	          if (ftxmsg.step.game < game_self.game.step.game) {
console.log("purging move...  that arrived while we were waiting...");
                    game_self.game.future.splice(ii, 1);
	          }
	        }
	      }
	...
	if (cont == 0) {
	  ...
	  // do we have future game moves waiting
	  //
console.log("2 -- WAITING ON FUTURE MOVES? " + game_self.game.future.length);
          if (game_self.game.future.length > 0 && game_self.gaming_active != 1 && game_self.game.halted != 1) {
            for (let ii = 0; ii < game_self.game.future.length; ii++) {
              let ftx = new saito.transaction(JSON.parse(game_self.game.future[ii]));
              let ftxmsg = ftx.returnMessage();
              if (ftxmsg.step.game == (game_self.game.step.game+1) && ftxmsg.game_id == game_self.game.id) {
                game_self.game.future.splice(ii, 1);
console.log(" 2 --- executing move that arrived while we were waiting...: " + JSON.stringify(ftx));
                this.handleGameMove(this.app, ftx, 1);
	      } else {
	        if (ftxmsg.step.game < game_self.game.step.game) {
console.log(" 2 --- purging move that arrived while we were waiting...");
                  game_self.game.future.splice(ii, 1);
	        }
	      }
	    }
