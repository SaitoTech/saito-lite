...
***REMOVED***);
  ***REMOVED***
  ...
  async returnPublicKey(addr_input) {
    if (this.app.crypto.isPublicKey(addr_input)) { return addr_input ***REMOVED***
    return await this.app.keys.fetchPublicKeyPromise(addr_input);
  ***REMOVED***
  ...
  ***REMOVED***
  ...
  async fetchIdentifiers(keys=[]) {
    try {
      ...
      console.log("FETCHED KEYS: ", answer);
