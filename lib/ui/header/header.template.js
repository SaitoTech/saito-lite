module.exports = HeaderTemplate = (ddmods) => {
  ...
  for (let i = 0; i < ddmods.length; i++) {
    html += `<a class="header-icon-fullscreen" href="${ddmods[i].returnLink()***REMOVED***"><i id="${ddmods[i].returnSlug()***REMOVED***" class="header-icon icon-med ${ddmods[i].icon_fa***REMOVED***"><div class="redicon"></div></i></a>`;
   ***REMOVED***
   html += `
