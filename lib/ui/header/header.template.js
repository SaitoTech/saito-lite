module.exports = HeaderTemplate = (ddmods) => {
  ...
  for (let i = 0; i < ddmods.length; i++) {
    html += `<a class="header-icon-fullscreen" href="${ddmods[i].returnLink()}"><i id="${ddmods[i].returnSlug()}" class="header-icon icon-med ${ddmods[i].icon_fa}"><div class="redicon"></div></i></a>`;
   }
   html += `
