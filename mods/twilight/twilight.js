...
class Twilight extends GameTemplate {
  ...
  constructor(app) {
    ...
    this.log_length = 150;
    this.interface = 1;
    this.dont_show_confirm_event = 0;
    this.dont_show_confirm_ops = 0;
    this.dont_show_confirm_space = 0;
  ...
  menuItems() {
    return {
      ...
      'game-deckhands': {
        name: 'Deck/Hands',
        callback: this.handleDeckhandsMenuItem.bind(this)
      },
      'game-lang': {
        name: 'Display',
    ...
    let twilight_self = this;
           ...
           let ussr_cards = this.game.deck[0].hand.length;
            ...
            if (this.game.player == 2) {
              let ussr_cards = this.game.state.opponent_cards_in_hand;
              confirm("Your opponent has "+ussr_cards+ " cards in hand, and there are "+this.game.deck[0].crypt.length +" cards remaining in deck");
            }
   
    if (this.game.player == 1){
      confirm("Your opponent has "+us_cards+ " cards in hand, and there are "+this.game.deck[0].crypt.length +" cards remaining in deck");
    }
...
initializeGame(game_id) {
  ...
  if (this.app.options != undefined) {
    if (this.app.options.gameprefs != undefined) {
      ...
      if (this.app.options.gameprefs.dont_show_confirm_event == 1) {
        this.dont_show_confirm_event = 1;
      }
      if (this.app.options.gameprefs.dont_show_confirm_ops == 1) {
        this.dont_show_confirm_ops = 1;
      }
      if (this.app.options.gameprefs.dont_show_confirm_space == 1) {
        this.dont_show_confirm_space = 1;
      }
    }
  }
  ...
  handleGameLoop(msg=null) {
    ...
    shd_continue = 1;
            ...
            if (twilight_self.countries[mv[2]].us < original_us) {
              ...
              if (valid_targets == 0) {
                ...
                if (twilight_self.game.player == 1) {
                  ...
                  let user_message = "Che takes effect. Pick first target for coup:<ul>";
        ...
        if (mv[0] === "teardownthiswall") {
          ...
          let user_message = "<span>Tear Down this Wall is played -- US may make 3 OP free Coup Attempt or Realignments in Europe.</span><ul>";
	      ...
	      return 1;
            ...
            $('.card').on('click', function() {
              ...
              if (action2 == "discard") {
                ...
                user_message += '</ul> </span>If you wish to cancel your discard,</span> <span class="card dashed" id="finished">click here</span>.';
                twilight_self.updateStatus(user_message);
    ...
    if (this.game.player == 2) { opponent = "ussr"; }
      ...
      $('.card').on('click', function() {
        ...
        if (action == "event") {
          ...
          // our event or both
          //
          if (twilight_self.dont_show_confirm_event == 0) {
            let fr =
              `
            ...
            $('input:checkbox').change(function() {
              if ($(this).is(':checked')) {
                twilight_self.dont_show_confirm_event = 1;
                twilight_self.saveGamePreference('dont_show_confirm_event', 1);
              }
            })
        ...
        if (action == "ops") {
          if (card != "missileenvy" || is_this_missile_envy_noneventable == 0){
            if (twilight_self.dont_show_confirm_ops == 0) {

              let fr =
                `
                <div>Confirm you want to play this card for ops</div>
              <ul>
                <li class="card" id="playops">play for ops</li>
                <li class="card" id="pickagain">pick again</li>
                </ul>
                <input type="checkbox" name="dontshowme" value="true" style="width: 20px;height: 1.5em;"> Don't show me this again
                `;
              ...
              $('.card').on('click', function() {
                ...
                $('input:checkbox').change(function() {
                  if ($(this).is(':checked')) {
                    twilight_self.dont_show_confirm_ops = 1;
                    twilight_self.saveGamePreference('dont_show_confirm_ops', 1);
                  }
                })
            ...
            // play for ops normally when not confirmed
        ...
        if (action == "space") {
          ...
          if (card != "missileenvy" || is_this_missile_envy_noneventable == 0){
            if (twilight_self.dont_show_confirm_space == 0) {
              let fr =
                `
                <div>Confirm you want to Space this card</div>
              <ul>
                <li class="card" id="space">Space</li>
                <li class="card" id="pickagain">pick again</li>
                </ul>
                <input type="checkbox" name="dontshowme" value="true" style="width: 20px;height: 1.5em;"> Don't show me this again
                `;
              ...
              $('input:checkbox').change(function() {
                if ($(this).is(':checked')) {
                  twilight_self.dont_show_confirm_space = 1;
                  twilight_self.saveGamePreference('dont_show_confirm_space', 1);
                }
              })
          ...
          // space normally when not confirmed
  ...
  playEvent(player, card) {
    ...
    if (card == "blockade") {
      ...
      if (this.game.player == 2) {
        ...
        this.updateStatus('<span>Blockade triggers:</span><ul><li class="card" id="discard">discard 3 OP card</li><li class="card" id="remove">remove all US influence in W. Germany</li></ul>');
    ...
    if (card == "olympic") {
      ...
      if (player == me) {
        ...
        twilight_self.updateStatus('<span>' + opponent.toUpperCase() + ' holds the Olympics:</span><ul><li class="card" id="boycott">boycott</li><li class="card" id="participate">participate</li></ul>');
...
console.log("card: " + card);
    ...
    if (card == "summit") {
      ...
      if (is_winner == 0) {
        ...
        if (my_go == 1) {
          ...
          this.updateStatus('<span>You win the Summit:</span><ul><li class="card" id="raise">raise DEFCON</li><li class="card" id="lower">lower DEFCON</li><li class="card" id="same">do not change</li></ul>');
    ...
    if (card == "southafrican") {
      ...
      if (this.game.player == 1) {
        ...
        twilight_self.updateStatus('USSR chooses:<ul><li class="card" id="southafrica">2 Influence in South Africa</li><li class="card" id="adjacent">1 Influence in South Africa and 2 Influence in adjacent countries</li></ul>');
    ...
    if (card == "debtcrisis") {
      ...
      let user_message = "<span>Choose a card to discard or USSR doubles influence in two countries in South America:</span><ul>";
      for (i = 0; i < this.game.deck[0].hand.length; i++) {
        if (this.modifyOps(this.game.deck[0].cards[this.game.deck[0].hand[i]].ops, this.game.deck[0].hand[i], this.game.player, 0) > 2 && this.game.deck[0].hand[i] != "china") {
          user_message += '<li class="card showcard" id="'+this.game.deck[0].hand[i]+'">'+this.game.deck[0].cards[this.game.deck[0].hand[i]].name+'</li>';
          cards_available++;
    ...
    if (card == "iraniraq") {
      ...
      if (me == player) {
        ...
        twilight_self.updateStatus('Iran-Iraq War. Choose Target:<ul><li class="card" id="invadeiraq">Iraq</li><li class="card" id="invadeiran">Iran</li></ul>');
